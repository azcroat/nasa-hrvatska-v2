...
function App(){
  ...
  useEffect(()=>{if(authScreen!=="app"||!authUser)return;function fetchIfNewer(){const lp=gP(authUser.u);const lpTs=(lp&&lp.savedAt)||0;fbLoadProgress(authUser.u).then(function(fp){if(!fp)return;const fpTs=fp._fbUpdated||fp.savedAt||0;if(fpTs>lpTs){sP(authUser.u,fp);const _pllSt=fp.stats||fp.st||{};setStats({...ds,..._pllSt});if(fp.name)setName(fp.name);applyRemoteProgress(fp);}}).catch(function(){/* network error — keep local state */})}// Fetch immediately on app load so returning users always get the latest Firebase data
