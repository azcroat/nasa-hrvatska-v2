...
function App(){
  ...
  useEffect(()=>{initFirebase();try{history.replaceState({scr:"welcome"},"",window.location.pathname)}catch(e){}
    const s=gS();if(s&&s.u){if(isSessionExpired()){cS();setAs("login");setTimeout(function(){setAe("\u2705 Your session expired. Your account is safe \u2014 just sign in again.")},200);return}const a=gA();if(a[s.u]){const p=gP(s.u);setAu({u:s.u,d:a[s.u].d,e:a[s.u].e||s.u});touchSession();updateStreak();var lf=getLocalFamily();if(lf)setFamData(lf);if(p){setName(p.name||a[s.u].d);setSt(p.st||ds);setScr((p.cp||(p.st&&(p.st.xp>0||p.st.lc>0)))?"dashboard":"welcome")}else setName(a[s.u].d);setAs("app");try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){};try{history.replaceState({scr:"dashboard"},"",window.location.pathname);history.pushState({scr:"dashboard"},"",window.location.pathname)}catch(ex){}}else{
      if(_fbReady&&_fbAuth){
        _fbAuth.onAuthStateChanged(function(user){
          if(user){var dn=user.displayName||user.email;var k=user.email;
            var a=gA();a[k]={d:dn,e:k};sA(a);
