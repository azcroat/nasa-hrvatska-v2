...
export function dbgInfo(...args: unknown[]): void  { const m = _fmt(...args); console.info('[DBG]', m);  _push('info',  m); }
export function dbgWarn(...args: unknown[]): void  { const m = _fmt(...args); console.warn('[DBG]', m);  _push('warn',  m); }
export function dbgError(...args: unknown[]): void { const m = _fmt(...args); console.error('[DBG]', m); _push('error', m); }

export function getEntries(): readonly LogEntry[] { return _entries; }
export function clearEntries(): void { _entries.length = 0; window.dispatchEvent(new Event('nh:debuglog')); }
