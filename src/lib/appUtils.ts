...
export function updateStreak(todayOverride?: string): StreakData & { milestone: number | null; freezeUsed?: boolean } {
  ...
  if (s.last === today) {
    try { const eb = JSON.parse(localStorage.getItem('nh_earn_back') || 'null'); if (eb && (eb.date === today || eb.date === yesterday)) { eb.lc = (eb.lc || 1) + 1; localStorage.setItem('nh_earn_back', JSON.stringify(eb)); } } catch {}
...
export function getStreakEarnBack(): EarnBackData | null {
  try {
    const eb = JSON.parse(localStorage.getItem('nh_earn_back') || 'null'); if (!eb) return null;
    const t = localDateStr(); const _yd = new Date(t + 'T00:00:00'); _yd.setDate(_yd.getDate() - 1);
    const yest = _yd.getFullYear() + '-' + String(_yd.getMonth() + 1).padStart(2, '0') + '-' + String(_yd.getDate()).padStart(2, '0');
...
export const BADGES: Badge[] = [
  ...
  { id: 'srs25',  n: 'Word Enthusiast',    i: '📖', d: 'Review 25 SRS words',                  r: s => (s.srsTotal || 0) >= 25 },
  ...
  { id: 'srs500', n: 'Word Hoarder',       i: '🗂️', d: 'Review 500 SRS words',                 r: s => (s.srsTotal || 0) >= 500 },
  { id: 'srs1k',  n: 'Lexicon Lord',       i: '🏅', d: 'Review 1,000 SRS words',               r: s => (s.srsTotal || 0) >= 1000 },
  { id: 'sp5',    n: 'Speaker',            i: '🗣️', d: 'Complete 5 speaking sessions',          r: s => (s.sp || 0) >= 5 },
  { id: 'sp10',   n: 'Fluent Voice',       i: '🎙️', d: 'Complete 10 speaking sessions',         r: s => (s.sp || 0) >= 10 },
  { id: 'sp25',   n: 'Orator',             i: '🏛️', d: 'Complete 25 speaking sessions',         r: s => (s.sp || 0) >= 25 },
  { id: 'gc5',    n: 'Grammar Adept',      i: '✍️', d: 'Complete 5 grammar exercises',           r: s => (s.gc || 0) >= 5 },
  { id: 'gc10',   n: 'Grammar Master',     i: '🎓', d: 'Complete 10 grammar exercises',          r: s => (s.gc || 0) >= 10 },
