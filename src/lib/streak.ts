...
export function canRepairStreak(): boolean {
  try {
    ...
    const streakRaw = JSON.parse(localStorage.getItem('uStreak') || '{"count":0,"last":""}') as {
      count?: number;
      last?: string;
    };
...
export function getRepairCost(): number {
  try {
    ...
    const raw = statsKey
      ? (JSON.parse(localStorage.getItem(statsKey) || '{}') as Record<string, unknown>)
      : {};
...
export function repairStreak(currentXP: number): RepairResult {
  ...
  try {
    ...
    let restoredCount = 1;
    try {
      const eb = JSON.parse(localStorage.getItem('nh_earn_back') || 'null') as {
        prev?: number;
      } | null;
      if (eb && (eb.prev || 0) > 0) {
