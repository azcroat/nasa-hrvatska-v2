...
function App() {
  ...
  const [onboarded, setOnboarded] = useState(() => { try { return localStorage.getItem('onboarded') === 'true'; } catch { return false; } });
  ...
  useEffect(() => {
    ...
    let lastSeen = null;
    try { lastSeen = localStorage.getItem('lastSeen'); } catch {}
    const now = Date.now();
    ...
    try { localStorage.setItem('lastSeen', String(now)); } catch {}
