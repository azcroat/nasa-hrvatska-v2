...
export default function CelebrationScene({
  ...
  return (
    <svg
      ...
      style={style}
      role="img"
      aria-label="Celebration illustration"
    >
      <style>{`
        @keyframes csDrop {
      ...
      {/* Stars */}
      {[
        { x: w * 0.08, y: h * 0.6, s: 16 },
        { x: w * 0.92, y: h * 0.55, s: 20 },
        { x: w * 0.5, y: h * 0.88, s: 18 },
      ].map((star, i) => (
        <text key={i} x={star.x} y={star.y} fontSize={star.s} textAnchor="middle">
          ⭐
        </text>
      ))}
      ...
      <g style={{ animation: 'csTrophy 0.5s cubic-bezier(0.34,1.56,0.64,1) both' }}>
        ...
        <rect
          ...
          y={tBase2}
          width={36}
          height={tBase3 - tBase2}
          rx={1.5}
          fill="url(#cs-baseGrad)"
        />
      ...
      {[
        ...
        <g
          key={i}
          style={{ animation: `csBurst 0.6s ${i === 0 ? '0s' : '0.2s'} ease-out forwards` }}
        >
