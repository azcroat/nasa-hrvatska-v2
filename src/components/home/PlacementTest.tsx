...
function SkillBar({ icon, label, score, rating }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 700, color: '#374151', width: 72, flexShrink: 0 }}>
        {label}
      </span>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, color: ratingColor, width: 72, flexShrink: 0 }}>
        {rating}
      </span>
    </div>
  );
...
export default function PlacementTest({
  ...
  const diff =
    ps >= Math.round(pq.length * 0.6)
      ? 'advanced'
      : ps >= Math.round(pq.length * 0.35)
        ? 'intermediate'
        : 'beginner';
