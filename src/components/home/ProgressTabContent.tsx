...
const CEFR_LEVELS = [
  { code: 'A1', label: 'Beginner',     minLessons: 0,   maxLessons: 8,   color: '#16a34a', bg: '#f0fdf4', border: '#86efac', desc: 'Basic greetings & survival phrases' },
  { code: 'A2', label: 'Elementary',   minLessons: 8,   maxLessons: 20,  color: '#0e7490', bg: '#f0f9ff', border: '#bae6fd', desc: 'Everyday conversations & simple needs' },
  { code: 'B1', label: 'Intermediate', minLessons: 20,  maxLessons: 45,  color: '#7c3aed', bg: '#f5f3ff', border: '#c4b5fd', desc: 'Travel, opinions & familiar topics' },
  { code: 'B1+', label: 'Upper-Inter', minLessons: 45,  maxLessons: 80,  color: '#d97706', bg: '#fffbeb', border: '#fcd34d', desc: 'Complex sentences & abstract ideas' },
  { code: 'B2', label: 'Upper-Inter+', minLessons: 80,  maxLessons: 9999, color: '#D40030', bg: '#fff0f2', border: '#fca5a5', desc: 'Fluent conversations with native speakers' },
];
...
function getVocabStats() {
  ...
  } catch { return { mastered: 0, seen: 0 }; }
}
...
function getStreakHistory() {
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    const practiced = localStorage.getItem('nh_practiced_' + key) === '1' ||
  ...
  return (
    <React.Fragment>
      ...
      <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:16, marginTop:8}}>
        ...
        <span style={{fontSize:'var(--text-sm)', fontWeight:800, color:'var(--heading)', letterSpacing:'0.08em', textTransform:'uppercase'}}>Your Progress</span>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--heading)', lineHeight: 1.1 }}>{cefrLevel.label}</div>
              <div style={{ fontSize: 11, color: 'var(--subtext)', marginTop: 1 }}>{cefrLevel.desc}</div>
            </div>
          </div>
          {nextCEFR && (
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: 10, color: 'var(--subtext)', fontWeight: 700 }}>Next level</div>
              <div style={{ fontSize: 13, fontWeight: 900, color: cefrLevel.color }}>{nextCEFR.code}</div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: '12px 16px 14px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            {CEFR_LEVELS.slice(0, -1).map((l, i) => (
              <div key={l.code} style={{ display: 'flex', flexDirection: 'column', alignItems: i === 0 ? 'flex-start' : i === CEFR_LEVELS.length - 2 ? 'flex-end' : 'center' }}>
                <div style={{
          ...
          {/* Main progress track */}
          <div style={{ position: 'relative', height: 10, background: 'var(--bar-bg)', borderRadius: 8, overflow: 'hidden', marginBottom: 8 }}>
            {/* Completed segments */}
          ...
          {nextCEFR ? (
            <div style={{ fontSize: 12, color: 'var(--subtext)', fontWeight: 600 }}>
              <span style={{ fontWeight: 800, color: cefrLevel.color }}>{cefrProgress}%</span> through {cefrLevel.code}
              {lessonsToNextCEFR > 0 && (
                <span style={{ opacity: 0.75 }}> · {lessonsToNextCEFR} lesson{lessonsToNextCEFR !== 1 ? 's' : ''} to {nextCEFR.code}</span>
              )}
            </div>
          ) : (
            <div style={{ fontSize: 12, fontWeight: 800, color: cefrLevel.color }}>🎓 Upper Intermediate — near-fluent level!</div>
          )}
        </div>
      </div>
      ...
      {vocabSeen > 0 && (
        <div style={{
          background: 'var(--card)', borderRadius: 16, marginBottom: 12,
          padding: '14px 16px',
          border: '1.5px solid var(--card-b)',
          boxShadow: '0 2px 8px rgba(0,0,0,0.04)',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--heading)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>🧠</span> Vocabulary Mastered
            </div>
            <div style={{ fontSize: 20, fontWeight: 900, color: '#7c3aed' }}>
              {vocabMastered}<span style={{ fontSize: 12, color: 'var(--subtext)', fontWeight: 600 }}>/{vocabSeen}</span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 8, alignItems: 'stretch' }}>
            {[
              { label: 'Mastered', value: vocabMastered, total: vocabSeen, color: '#7c3aed', emoji: '✅' },
              { label: 'Learning', value: vocabSeen - vocabMastered, total: vocabSeen, color: '#0e7490', emoji: '📖' },
            ].map(item => (
              <div key={item.label} style={{ flex: 1, background: `${item.color}0f`, borderRadius: 10, padding: '8px 10px', border: `1px solid ${item.color}22` }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--subtext)', marginBottom: 3 }}>{item.emoji} {item.label}</div>
          ...
          <div style={{ fontSize: 11, color: 'var(--subtext)', marginTop: 6, fontWeight: 600 }}>
            {Math.round(vocabMastered / vocabSeen * 100)}% retention rate · review 3+ times to master
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--heading)', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <div key={key} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              <div style={{
                ...
                background: practiced
                  ...
                  : isToday ? 'rgba(14,116,144,0.15)' : 'var(--bar-bg)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <div style={{ width: 10, height: 10, borderRadius: 3, background: 'linear-gradient(135deg,#16a34a,#22c55e)' }} />
            <span style={{ fontSize: 10, color: 'var(--subtext)', fontWeight: 600 }}>Practiced</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <div style={{ width: 10, height: 10, borderRadius: 3, background: 'var(--bar-bg)', border: '1px solid var(--card-b)' }} />
