...
export default function GrammarDiagnosisScreen({
  ...
  async function generate() {
    ...
    let writingMistakes = [];
    try {
      writingMistakes = JSON.parse(localStorage.getItem('nh_writing_mistakes') || '[]').slice(
        0,
        20,
      );
    } catch (_) {}
  ...
  function getDataCounts() {
    ...
    const majaCount = Array.isArray(majaMemory.mistakePatterns)
      ? majaMemory.mistakePatterns.length
      : 0;
    let writingCount = 0;
    try {
      writingCount = JSON.parse(localStorage.getItem('nh_writing_mistakes') || '[]').length;
    } catch (_) {}
      ...
      <div className="scr-wrap">
        ...
        <div className="c" style={{ marginBottom: 20, padding: '20px 18px' }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 14 }}>
            ...
            <div>
              <div
                style={{ fontWeight: 700, color: 'var(--heading)', fontSize: 16, marginBottom: 4 }}
              >
                Your Personal Grammar Report
              </div>
              <div style={{ fontSize: 14, color: 'var(--subtext)', lineHeight: 1.55 }}>
                Claude analyses your SRS mistakes, Maja conversation errors, and writing corrections
                to pinpoint the exact grammar rules you struggle with — then gives you targeted
                drills to fix them.
              </div>
            </div>
          </div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {[
              ...
              {
                icon: '✍️',
                label: `${writingCount} writing corrections`,
                active: writingCount > 0,
              },
            ].map((chip) => (
              <div
                key={chip.label}
        ...
        {hasData ? (
          ...
          <div className="c" style={{ textAlign: 'center', padding: '28px 20px' }}>
            ...
            <div
              style={{ fontWeight: 700, color: 'var(--heading)', marginBottom: 8, fontSize: 15 }}
            >
              Not enough data yet
            </div>
            <div
              style={{ fontSize: 14, color: 'var(--subtext)', lineHeight: 1.6, marginBottom: 20 }}
            >
              Practice more to unlock your diagnosis — try Flashcards, Maja, and Free Writing first!
            </div>
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 14, color: 'var(--subtext)', marginBottom: 36, lineHeight: 1.6 }}>
          Claude is reading your entire learning history
          <br />
          and identifying your grammar blind spots
        </div>
          ...
          return (
            ...
            >
              ...
              <button
                ...
                style={{
                  width: '100%',
                  background: isOpen ? sev.bg : 'var(--card)',
                  border: 'none',
                  cursor: 'pointer',
                  padding: '14px 16px',
                  display: 'flex',
                                    ...
                                    >
                                      <span
                                        style={{
                                          ...
                                          fontWeight: 800,
                                          color:
                                            answered && (isCorrect || (isChosen && !isCorrect))
                                              ? '#fff'
                                              : 'var(--subtext)',
                                          transition: 'all 0.15s',
                                        }}
