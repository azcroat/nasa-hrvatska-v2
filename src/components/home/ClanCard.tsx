  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div
          style={{ height: 5, background: 'var(--bar-bg)', borderRadius: 3, overflow: 'hidden' }}
        >
          <div
            style={{
              height: '100%',
    ...
    try {
      const res = await apiFetch('/api/clan', {
        ...
        body: JSON.stringify({
          action: 'join',
          uid,
          displayName: displayName || 'Učenik',
          clanId: code,
        }),
      });
      const data = await res.json();
        ...
        {phase === 'creating' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                onClick={createClan}
                disabled={submitting || !clanName.trim()}
                style={{
                  flex: 2,
                  height: 40,
                  background: '#7c3aed',
    ...
    >
      ...
      <div style={{ padding: '4px 16px 8px' }}>
        ...
        {spots > 0 && (
          ...
          >
            {spots} open spot{spots > 1 ? 's' : ''} — share your code:{' '}
            <span style={{ fontWeight: 800, fontStyle: 'normal', color: '#7c3aed' }}>
              {clan?.id}
            </span>
          </div>
        )}
