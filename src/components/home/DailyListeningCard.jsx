...
export default function DailyListeningCard({ level, award }) {
  ...
  const alreadyDone = phase !== 'done' && (() => { try { return localStorage.getItem(completedKey) === '1'; } catch { return false; } })();
  ...
  const handleAnswer = useCallback((qIdx, opt) => {
    if (checked) return;
  ...
  if (alreadyDone && phase !== 'done') {
    return (
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--heading)' }}>Daily Listening · Complete</div>
          <div style={{ fontSize: 12, color: 'var(--subtext)', marginTop: 2 }}>Come back tomorrow for a new exercise</div>
        </div>
        <div style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 700, color: '#16a34a' }}>✓ Done</div>
      </div>
    );
  }
  ...
  if (phase === 'idle') {
    return (
      <div style={{
        background: 'linear-gradient(135deg, rgba(14,116,144,0.08) 0%, rgba(12,74,110,0.05) 100%)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, fontWeight: 900, color: '#0e7490', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 2 }}>
              Daily Listening · {level}
            </div>
            <div style={{ fontSize: 13, color: 'var(--heading)', fontWeight: 600, lineHeight: 1.4 }}>
              {error || `Today's topic: ${getDailyTopic().charAt(0).toUpperCase() + getDailyTopic().slice(1)} · Dialogue with questions`}
            </div>
          </div>
          <button
            onClick={load}
            style={{
              flexShrink: 0, background: '#0e7490', color: '#fff',
        ...
        {error && (
          <div style={{ marginTop: 8, fontSize: 11, color: '#b91c1c', fontWeight: 600 }}>{error}</div>
        )}
      </div>
    );
  }
  ...
  if (phase === 'loading') {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--subtext)' }}>Generating today's Croatian dialogue…</div>
      </div>
    );
  }
  ...
  if (phase === 'done') {
    ...
    return (
      <div style={{
        background: 'var(--card)', border: '1.5px solid rgba(22,163,74,0.3)',
        borderRadius: 16, padding: '16px', marginBottom: 12, textAlign: 'center',
      }}>
        <div style={{ fontSize: 32, marginBottom: 6 }}>🎧</div>
        <div style={{ fontSize: 18, fontWeight: 900, color: score === total ? '#16a34a' : '#0e7490', marginBottom: 4 }}>
  ...
  return (
    <div style={{
      background: 'var(--card)', border: '1.5px solid rgba(14,116,144,0.25)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 900, color: '#fff', textTransform: 'uppercase', letterSpacing: '.08em' }}>
            Daily Listening · {level}
          </span>
        </div>
      ...
      <div style={{ padding: '12px 16px', maxHeight: 240, overflowY: 'auto' }}>
        <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--subtext)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 10 }}>
        ...
        {allLines.map((line, i) => (
          <div
            ...
            style={{
              display: 'flex', gap: 10, marginBottom: 10, cursor: 'pointer',
              padding: '8px 10px', borderRadius: 10,
              background: speakingLine === line.text ? 'rgba(14,116,144,0.12)' : 'var(--bar-bg)',
              border: speakingLine === line.text ? '1px solid rgba(14,116,144,0.3)' : '1px solid transparent',
              transition: 'background .2s ease',
            }}
          >
            <div style={{ fontSize: 11, fontWeight: 800, color: '#0e7490', minWidth: 60, flexShrink: 0, paddingTop: 1 }}>
              {line.speaker}
            </div>
            <div style={{ fontSize: 13, color: 'var(--heading)', lineHeight: 1.5, flex: 1 }}>
              {line.text}
              {speakingLine === line.text && <span style={{ marginLeft: 6, animation: 'pulse 1s ease-in-out infinite', fontSize: 11 }}>🔊</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      {data?.questions?.length > 0 && (
        <div style={{ padding: '0 16px 16px' }}>
          <div style={{ borderTop: '1px solid var(--card-b)', paddingTop: 12, marginBottom: 10 }}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--subtext)', textTransform: 'uppercase', letterSpacing: '.08em' }}>
              ❓ Comprehension Questions
            </div>
          </div>
          {data.questions.map((q, qi) => (
            <div key={qi} style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--heading)', marginBottom: 8, lineHeight: 1.4 }}>
                {qi + 1}. {q.question}
              </div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        fontFamily: "'Outfit',sans-serif", cursor: checked ? 'default' : 'pointer',
                        background: isCorrect ? 'rgba(22,163,74,0.15)' : isWrong ? 'rgba(220,38,38,0.1)' : isSelected ? 'rgba(14,116,144,0.1)' : 'var(--bar-bg)',
                        border: isCorrect ? '1.5px solid #16a34a' : isWrong ? '1.5px solid #dc2626' : isSelected ? '1.5px solid #0e7490' : '1.5px solid transparent',
          ...
          {!checked && Object.keys(answers).length < data.questions.length && (
            <div style={{ fontSize: 11, color: 'var(--subtext)', textAlign: 'center', fontWeight: 600 }}>
