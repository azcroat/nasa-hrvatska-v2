...
import { CONDITIONAL, FORMAL_REGISTER, IMPERSONAL, TECH_VOC, BUREAUCRATIC } from '../../data.jsx';
...
export function ImpersonalScreen({ goBack, award }) {
  ...
  const tabs = [{ k: 'constructions', l: 'Constructions' }, { k: 'signs', l: 'Signs' }, { k: 'quiz', l: 'Quiz' }];
  ...
  return (
    <div className="scr-wrap">
      ...
      {tab === 'signs' && (
        <div>
          <div style={{ marginBottom: 12, padding: '10px 14px', background: 'rgba(14,116,144,.07)', borderRadius: 10, fontSize: 12, color: '#0e7490', lineHeight: 1.6 }}>
            These signs appear on buildings, shops, and public spaces across Croatia. Recognising them instantly will save you a lot of confusion.
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {IMPERSONAL.signs.map(function (s, i) {
              return (
                <button key={i} style={{ padding: '14px 12px', borderRadius: 14, border: '2px solid #e2e8f0', background: 'white', cursor: 'pointer', textAlign: 'center', fontFamily: "'Outfit',sans-serif", boxShadow: '0 1px 3px rgba(0,0,0,.05)' }} onClick={() => speak(s.sign)}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#0e7490', letterSpacing: '.03em' }}>{s.sign}</div>
  ...
  return (
    <div className="scr-wrap">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
        ...
        <button className={'b ' + (tab === 'phrases' ? 'bp' : 'bg')} style={{ fontSize: 13 }} onClick={() => setTab('phrases')}>Phrases</button>
        <button className={'b ' + (tab === 'quiz' ? 'bp' : 'bg')} style={{ fontSize: 13 }} onClick={() => setTab('quiz')}>Quiz</button>
      </div>
        ...
        <div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
            {TECH_VOC.categories.map(function (c, i) {
              return (
                <button key={i} className={'b ' + (catIdx === i ? 'bp' : 'bg')} style={{ fontSize: 12 }} onClick={() => setCatIdx(i)}>
...
export function BureaucraticScreen({ goBack, award }) {
  ...
  return (
    <div className="scr-wrap">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
        <button className={'b ' + (tab === 'vocab' ? 'bp' : 'bg')} style={{ fontSize: 13 }} onClick={() => setTab('vocab')}>Vocabulary</button>
        <button className={'b ' + (tab === 'phrases' ? 'bp' : 'bg')} style={{ fontSize: 13 }} onClick={() => setTab('phrases')}>Key Phrases</button>
        <button className={'b ' + (tab === 'quiz' ? 'bp' : 'bg')} style={{ fontSize: 13 }} onClick={() => setTab('quiz')}>Quiz</button>
      </div>
      ...
      {tab === 'phrases' && (
        <div>
          <div style={{ marginBottom: 12, padding: '10px 14px', background: 'rgba(14,116,144,.07)', borderRadius: 10, fontSize: 12, color: '#0e7490', lineHeight: 1.6 }}>
            💡 Tip: Use Vi (formal address) in all government offices, healthcare facilities, and with landlords.
          </div>
