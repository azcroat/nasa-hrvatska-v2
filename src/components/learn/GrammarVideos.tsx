  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <span style={{ fontSize: 'var(--text-base)', fontWeight: 800, color: 'var(--heading)' }}>
            {ch.name}
          </span>
          <span
            style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ flex: 1, fontSize: 'var(--text-sm)', fontWeight: 800, color: 'var(--heading)' }}
        >
          {topic.title}
        </span>
        <span
...
export default function GrammarVideos({
  ...
  const filtered = levelFilter === 'All' ? TOPICS : TOPICS.filter((t) => t.level === levelFilter);
  ...
  return (
    <div className="scr-wrap">
      ...
      >
        Curated YouTube resources for every grammar topic — from Croatian alphabet to advanced pitch
        accent. Each card links to live YouTube search results so you always find the freshest,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
        {filtered.map((topic) => (
          <TopicCard key={topic.key} topic={topic} onInApp={(screen) => setScr && setScr(screen)} />
        ))}
      </div>
