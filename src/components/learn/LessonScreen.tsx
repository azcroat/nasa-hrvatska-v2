...
export default function LessonScreen({
  ...
  useEffect(() => {
    if (lp === 'learn') {
      const tips = [
        {
          mood: 'happy',
          text: 'New words! Tap each one to hear the pronunciation — then say it back out loud. Hearing yourself is the shortcut. 🔊',
        },
        {
          mood: 'ready',
          text: 'Vocabulary is the bricks of language. Learn these words and your Croatian conversations open up. 🏰',
        },
        {
          mood: 'thinking',
          text: 'Pro tip: connect each word to a vivid image. "Kuća" — picture a house you know. Croatian sticks when it has meaning. 🧠',
        },
        {
          mood: 'encouraging',
    ...
    knightSpeak(
      ...
      pct === 1
        ? 'Savršeno! That lesson is yours forever now. 🌟'
        : pct >= 0.7
          ? `Odlično! ${Math.round(pct * 100)}% — real Croatian progress. 💪`
  ...
  const flashPool = (li || [])
    .map((w) => [w.w || w[0], w.e || w[1], w.p || w[2], w.ex || w[3]])
  ...
  /* ── FLASHCARDS OVERLAY ───────────────────────────────────────── */
  if (showFlashcards) {
    return <Flashcards pool={flashPool} goBack={() => setShowFlashcards(false)} award={awardFn} />;
  }
    ...
    return (
      <div className="scr-wrap">
        ...
        >
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--info)' }}>
              TODAY'S LESSON
            </div>
            ...
            onClick={() => {
              ...
              const q = qPool.map((w, i) => {
                ...
                if (isCrToEn) {
                  ...
                  const wr = sh(li.filter((x) => x[1] !== w[1]))
                    .slice(0, 3)
                    .map((x) => x[1]);
                  const o = sh([w[1], ...wr]);
                  ...
                  const wr = sh(li.filter((x) => x[0] !== w[0]))
                    .slice(0, 3)
                    .map((x) => x[0]);
                  const o = sh([w[0], ...wr]);
      ...
      <div className="scr-wrap">
        {showQuit && (
          ...
          >
            <div
              className="c"
              style={{ maxWidth: 320, width: '100%', padding: 24, textAlign: 'center' }}
            >
              ...
              <div
                style={{ fontSize: 'var(--text-sm)', color: 'var(--subtext)', marginBottom: 20 }}
              >
        ...
        <div className="c" style={{ marginTop: 16 }}>
          ...
          {la && !isCorrect && (
            ...
            >
              ...
              {qi?.[lx]?.hint ||
                ...
                (qi?.[lx]
                  ? qi[lx].direction === 'en-cr'
                    ? `"${qi[lx][1]}" translates to "${qi[lx][0]}" in Croatian.`
                    : 'Review this word — it will come up again in spaced repetition.'
                  : 'Review this word — it will come up again in spaced repetition.')}
            </div>
          )}
          ...
          {la && (
            ...
            >
              {lx < qi.length - 1 ? 'Next →' : 'See Results'}
            </button>
          )}
        </div>
      </div>
      ...
      >
        ...
        <div
          style={{
            ...
            bottom: 0,
            background:
              'linear-gradient(105deg, transparent 25%, rgba(255,255,255,.04) 50%, transparent 75%)',
        ...
        >
          ...
          <div
            style={{
              ...
              filter:
                scorePct === 1
                  ? 'drop-shadow(0 0 20px rgba(251,191,36,.8))'
                  : 'drop-shadow(0 0 12px rgba(74,222,128,.5))',
            }}
          >
            ...
            {[
              {
                ...
                action: () => setShowFlashcards(true),
              },
              {
                icon: '🔁',
                title: 'SRS Review',
                desc: 'Lock it in long-term',
                action: () => {
                  goBack();
                  if (typeof setScr === 'function') setTimeout(() => setScr('review'), 50);
                },
              },
              {
                icon: '🎯',
                title: 'Quick Quiz',
                desc: 'Test yourself now',
                action: () => {
                  if (typeof goToPractice === 'function') goToPractice();
                  else goBack();
                },
              },
            ].map((ex) => (
