...
export default function BrowseContentModal({
  allCats, icons, st, setScr, sCurEx,
  sGl, sGp, sGx, sGs, sGa, sGsl,
  launchVocab, launchAnimLesson,
  onClose,
}) {
  return (
    <div style={{
      ...
      <div style={{ padding:'0 16px 40px' }}>
          ...
          <Section title="Vocabulary" icon="📚" count={`${allCats.length + 6} topics`} defaultOpen={true}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
              {allCats.map(t => {
                ...
                return (
                  <button key={t} className="tc" style={{ textAlign:"center", padding:"14px 8px" }} onClick={() => { onClose(); launchVocab(t); }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
              {[
                ...
                <button key={screen} className="tc" style={{ textAlign:"center", padding:"14px 8px" }} onClick={() => { onClose(); setScr(screen); }}>
          ...
          <Section title="Grammar" icon="📝" count="14 lessons" defaultOpen={true}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
              {[
                ...
                <button key={label} className="tc" style={{ display:"flex", alignItems:"center", gap:10, padding:"13px 14px", textAlign:"left" }} onClick={() => { onClose(); fn(); }}>
        ...
        <div id="learn-section-reading">
          <Section title="Reading" icon="📖" count="100+ passages" defaultOpen={true}>
            <button className="tc" style={{ width:"100%", display:"flex", alignItems:"center", gap:14, padding:"16px" }} onClick={() => { onClose(); setScr("readlist"); }}>
          ...
          <Section title="Grammar Videos & AI Lessons" icon="🎥" count="18 videos + AI" defaultOpen={true}>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              <button className="tc" style={{ width:"100%", display:"flex", alignItems:"center", gap:14, padding:"16px" }}
                onClick={() => { onClose(); setScr("grammarvideos"); }}>
              ...
              <button className="tc" style={{ width:"100%", display:"flex", alignItems:"center", gap:14, padding:"16px" }}
                onClick={() => { onClose(); setScr("grammarexplainer"); }}>
                <span style={{ fontSize:28 }}>🤖</span>
                <div style={{ textAlign:"left" }}>
        ...
        <Section title="Quick Reference" icon="📌" count="13 guides" defaultOpen={false}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
            {[
              ...
              <button key={screen} className="tc" style={{ textAlign:"center", padding:"12px 8px" }} onClick={() => { onClose(); setScr(screen); }}>
                <div style={{ fontSize:'var(--text-2xl)' }}>{icon}</div>
                <div style={{ fontSize:'var(--text-sm)', fontWeight:700, marginTop:4 }}>{label}</div>
              </button>
            ))}
