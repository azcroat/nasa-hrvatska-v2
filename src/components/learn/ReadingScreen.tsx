    ...
    <div className="scr-wrap">
      ...
      {rph === 'read' && (
        <React.Fragment>
          ...
          <button className="b bp" style={{ width: '100%' }} onClick={() => sRph('vocab')}>
            Study Vocabulary →
          </button>
        </React.Fragment>
      )}
        ...
        <React.Fragment>
          ...
          <div className="c" style={{ marginTop: 8 }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!ra) {
                        sRsl(i);
                        sRa(true);
                        if (i === rp.qs[rqi].c) sRsc((s) => s + 1);
                      }
                    }}
                  >
                    {ra && isCorrect ? '✓ ' : ra && isSelected && !isCorrect ? '✗ ' : ''}
                    {o}
                  </button>
                );
              })}
            ...
            {ra && (
              ...
              >
                {rqi < rp.qs.length - 1 ? 'Next →' : 'Results'}
              </button>
            )}
          </div>
        </React.Fragment>
      ...
      {rph === 'result' && (
        <div style={{ textAlign: 'center', paddingTop: 40 }}>
          <div style={{ fontSize: 64 }}>
            {rsc === rp.qs.length ? '🌟' : rsc >= Math.ceil(rp.qs.length / 2) ? '📖' : '💪'}
          </div>
