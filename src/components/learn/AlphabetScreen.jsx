...
export default function AlphabetScreen({ goBack, award }) {
  ...
  function handleAnswer(opt) {
    ...
    setSelected(opt);
    setAnswered(true);
    if (isCorrect) { setScore(s => s + 1); speak(q.prompt); }
  }
  ...
  if (mode === 'reference') {
    return (
      <div className="scr-wrap">
        ...
        {learnedCount > 0 && (
          <div style={{marginBottom:12,padding:"8px 14px",background:"rgba(22,163,74,.08)",borderRadius:10,fontSize:12,color:"#15803d",fontWeight:700}}>
        ...
        <div className="c" style={{marginBottom:12,padding:"10px 14px",background:"rgba(14,116,144,.06)",fontSize:12}}>
          💡 Croatian spelling is <strong>100% phonetic</strong> — every letter always makes exactly one sound. Tap each letter to hear it.
        </div>

        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {ALPHA.map((l,i)=>(
            <button key={i} className="c" style={{
              padding:"12px 16px",
              border: learnedRef.current.has(l[0]) ? "2px solid #16a34a" : "1px solid var(--card-b)",
              background: learnedRef.current.has(l[0]) ? "rgba(22,163,74,.05)" : "var(--card)",
            }} onClick={()=>{ speak(l[2]); markLearned(l[0]); }}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:22,fontWeight:800,color:learnedRef.current.has(l[0])?"#16a34a":"#164e63",fontFamily:"monospace",minWidth:55}}>
                  {l[0]}{learnedRef.current.has(l[0])?" ✓":""}
                </span>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:12,color:"#0e7490",fontWeight:700}}>{l[1]}</div>
                  <div style={{fontSize:13}}>{l[2]} <span style={{color:"#78716c",fontSize:11}}>({l[3]})</span></div>
                </div>
              </div>
            </button>
          ))}
        ...
        <div style={{marginTop:16,marginBottom:24}}>
          <button className="b bp" style={{width:"100%",fontSize:15}} onClick={()=>setMode('quiz')}>
            🎯 Test the Alphabet →
          </button>
        </div>
      </div>
    ...
    return (
      <div className="scr-wrap">
        ...
        <div style={{textAlign:"center",padding:"32px 16px"}}>
          <div style={{fontSize:48,marginBottom:12}}>{pct>=0.9?"🏆":pct>=0.7?"⭐":"💪"}</div>
          ...
          <div style={{fontSize:13,color:"#78716c",marginBottom:24}}>
            {pct>=0.9?"You know the Croatian alphabet! Each letter → one sound.":pct>=0.7?"Good work! Practice the tricky ones: č, š, ž, lj, nj.":"Review the letters and try again — it gets easier fast!"}
          </div>
  ...
  return (
    <div className="scr-wrap">
      ...
      <div className="c" style={{marginBottom:16,padding:"20px 16px",textAlign:"center"}}>
        <div style={{fontSize:12,color:"#78716c",marginBottom:8,fontWeight:700,letterSpacing:".05em"}}>WHICH LETTER SOUNDS LIKE THIS?</div>
        <div style={{fontSize:13,color:"#78716c",marginBottom:6}}>The word <strong style={{color:"#164e63"}}>{q.prompt}</strong> ({q.promptEn}) starts with the sound:</div>
        <button onClick={()=>speak(q.prompt)} style={{fontSize:18,fontWeight:900,color:"#0e7490",
          background:"rgba(14,116,144,.08)",border:"2px solid rgba(14,116,144,.2)",
          borderRadius:12,padding:"12px 28px",cursor:"pointer",fontFamily:"'Outfit',sans-serif"}}>
          🔊 {q.ipa}
        </button>
      </div>
        ...
        {q.opts.map((o,oi) => {
          ...
          return (
            <button key={oi} onClick={() => handleAnswer(o)}
              style={{padding:"18px 8px",border:`2px solid ${bc}`,borderRadius:14,background:bg,
                color:col,fontSize:22,fontWeight:800,cursor:answered?"default":"pointer",
                fontFamily:"monospace",transition:"all .15s",textAlign:"center"}}>
              {o}
            </button>
          );
        })}
