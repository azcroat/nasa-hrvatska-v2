...
export default function PadezifullScreen({ goBack, award }) {
  ...
  function startQuiz() {
    ...
    sPfQ(q); sPfI(0); sPfS(0); sPfA(false); sPfSl(-1); sPfO(q[0].opts); sPfCaseA(false); sPfCaseSl(-1);
  }

  const genderBg = g => g === "f" ? "linear-gradient(135deg,#dc2626,#b91c1c)" : g === "n" ? "linear-gradient(135deg,#16a34a,#15803d)" : "linear-gradient(135deg,#0e7490,#164e63)";
  ...
  return (
    <div className="scr-wrap">
        ...
        {["sing","plur","quiz"].map(t => (
          <button
            ...
            onClick={() => {
              ...
              if (t === "quiz") { sPfMode("quiz"); startQuiz(); }
            }}>
      ...
      {pfTab === "sing" && pfMode === "learn" && (() => {
        const data = PADEZI_FULL.singEndings[pfGender];
        return (
          <React.Fragment>
            <div style={{display:"flex",gap:6,marginBottom:16}}>
              {["f","n","m"].map(g => (
                <button key={g} className={"b " + (pfGender === g ? "bw" : "bg")} style={{fontSize:12,padding:"6px 14px"}} onClick={() => sPfGender(g)}>
                  {g === "f" ? "🔴 Ženski" : g === "n" ? "🟢 Srednji" : "🔵 Muški"}
                </button>
              ))}
            </div>
            <div className="c" style={{marginBottom:12,borderLeft:"4px solid " + genderBorder(pfGender)}}>
              <div style={{fontSize:15,fontWeight:800}}>{data.label}</div>
              <div style={{fontSize:13,color:"#78716c",marginTop:4}}>Endings: {data.endings.join(" / ")}</div>
              {data.note && <div style={{fontSize:12,color:"#b45309",marginTop:6,fontWeight:600}}>⚠️ {data.note}</div>}
            </div>
            {data.words.map((w, wi) => (
              <div key={wi} className="c" style={{marginBottom:12,padding:0,overflow:"hidden"}}>
                <div style={{padding:"10px 14px",background:genderBg(pfGender),color:"#fff",fontWeight:700,fontSize:14}}>
                  {w.nom} ({w.en})
                </div>
      ...
      {pfTab === "plur" && pfMode === "learn" && (() => {
        ...
        return (
          <React.Fragment>
            ...
            <div className="c" style={{marginBottom:12,borderLeft:"4px solid " + genderBorder(pfGender)}}>
              <div style={{fontSize:15,fontWeight:800}}>{data.label}</div>
              <div style={{fontSize:13,color:"#78716c",marginTop:4}}>Noun: {data.endings.join(" / ")}</div>
              <div style={{fontSize:13,color:"#7c3aed",marginTop:2}}>Adj: {data.adjEnd.join(" / ")}</div>
            </div>
            {data.words.map((w, wi) => (
              <div key={wi} className="c" style={{marginBottom:12,padding:0,overflow:"hidden"}}>
                <div style={{padding:"10px 14px",background:genderBg(pfGender),color:"#fff",fontWeight:700,fontSize:14}}>
      ...
      {pfMode === "quiz" && (() => {
        ...
        if (!pfA) return (
          <React.Fragment>
            ...
            <div className="c" style={{marginTop:16}}>
              <div style={{fontSize:12,color:"#b45309",fontWeight:600}}>Base: {q.base}</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:16}}>
              {pfO.map((o, oi) => (
                <button
                  key={oi}
                  className="ob"
                  onClick={() => { sPfSl(oi); sPfA(true); if (o === q.answer) sPfS(s => s + 1); }}>
                  {o}
                </button>
              ))}
            </div>
        ...
        if (pfA && !pfCaseA) return (
          <React.Fragment>
            <div className="c" style={{textAlign:"center",marginTop:16}}>
              <div style={{fontSize:16,fontWeight:700,color:pfO[pfSl] === q.answer ? "#16a34a" : "#dc2626"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:16,justifyContent:"center"}}>
              {PADEZI_FULL.caseNames.map((cn, ci) => (
                <button
                  ...
                  onClick={() => { if (!pfCaseA) { sPfCaseSl(ci); sPfCaseA(true); if (cn === q.caseName && pfO[pfSl] === q.answer) sPfS(s => s + 0.5); } }}>
                  {cn}
                </button>
              ))}
            </div>
        ...
        return (
          <div style={{textAlign:"center",marginTop:16}}>
            <div style={{fontSize:16,fontWeight:700}}>{q.answer} ({q.caseName})</div>
            <button
              className="b bp"
              style={{marginTop:16}}
              onClick={() => {
                if (pfI < total - 1) {
                  const n = pfQ[pfI + 1];
                  sPfO(n.opts); sPfI(pfI + 1); sPfA(false); sPfSl(-1); sPfCaseA(false); sPfCaseSl(-1);
                } else {
                  sPfI(total);
                }
              }}>
