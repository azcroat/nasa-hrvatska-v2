...
const TOPICS = [
  ...
  { id:"present",   icon:"🔧", title:"Present Tense",       level:"A1", desc:"Verb conjugation basics" },
  { id:"past",      icon:"⏪", title:"Past Tense",          level:"A2", desc:"Perfekt with biti + l-form" },
  { id:"future",    icon:"⏩", title:"Future Tense",        level:"A2", desc:"Two ways to express the future" },
  { id:"cases_intro",icon:"📐",title:"Cases Overview",      level:"A2", desc:"What the 7 padeži do" },
  { id:"nominative",icon:"①", title:"Nominative Case",     level:"A2", desc:"Subjects and complements" },
  { id:"accusative",icon:"②", title:"Accusative Case",     level:"A2", desc:"Direct objects" },
  { id:"genitive",  icon:"③", title:"Genitive Case",       level:"B1", desc:"Possession, negation, quantity" },
  { id:"dative",    icon:"④", title:"Dative Case",         level:"B1", desc:"Indirect objects, to/for" },
  { id:"locative",  icon:"⑤", title:"Locative Case",       level:"B1", desc:"Location after u/na/o/po" },
  { id:"instrumental",icon:"⑥",title:"Instrumental Case",  level:"B1", desc:"With/by means of" },
  { id:"vocative",  icon:"⑦", title:"Vocative Case",       level:"B1", desc:"Direct address" },
  { id:"aspect",    icon:"↔️", title:"Verb Aspect",         level:"B1", desc:"Perfective vs imperfective" },
  { id:"clitics",   icon:"📎", title:"Clitic Pronouns",    level:"B1", desc:"Second-position rule" },
...
export default function GrammarExplainer({ goBack, award }) {
  ...
  const [level, setLevel] = useState(() => {
    try { return localStorage.getItem("nh_placement_result") || "A2"; }
    catch { return "A2"; }
  });
  ...
  if (phase === "lesson" && lesson) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "var(--app-bg)", paddingBottom: 80 }}>
        ...
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 16px", display: "flex", flexDirection: "column", gap: 20 }}>

          {/* 1. Header card */}
          <div style={{ background: "linear-gradient(135deg, var(--accent) 0%, #7c3aed 100%)", borderRadius: 16, padding: "24px 20px", color: "#fff" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              <span style={{ fontSize: 32 }}>{selectedTopic?.icon}</span>
              <div>
                <div style={{ fontSize: "var(--text-xl)", fontWeight: 700 }}>{lesson.title || selectedTopic?.title}</div>
                <span style={{ background: "rgba(255,255,255,0.25)", borderRadius: 20, padding: "2px 10px", fontSize: "var(--text-xs)", fontWeight: 600 }}>{level}</span>
              </div>
            </div>
            <p style={{ margin: 0, fontSize: "var(--text-base)", fontStyle: "italic", opacity: 0.92, lineHeight: 1.55 }}>{lesson.intro}</p>
          </div>

          {/* 2. The Rule */}
          <div style={{ background: "var(--card)", border: "1px solid var(--card-b)", borderRadius: 14, padding: "20px 20px" }}>
            <div style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--heading)", marginBottom: 12 }}>📖 The Rule</div>
            <div style={{ background: "var(--bar-bg)", borderLeft: "4px solid var(--accent)", borderRadius: "0 10px 10px 0", padding: "14px 16px", fontSize: "var(--text-base)", color: "var(--heading)", lineHeight: 1.65 }}>
              {lesson.rule}
            </div>
          </div>
          ...
          {/* 3. Examples */}
          <div style={{ background: "var(--card)", border: "1px solid var(--card-b)", borderRadius: 14, padding: "20px 20px" }}>
            <div style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--heading)", marginBottom: 14 }}>📝 Examples</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {(lesson.examples || []).map((ex, i) => (
                <div key={i} className="example-card" style={{ background: "var(--bar-bg)", borderRadius: 12, padding: "14px 16px", animationDelay: `${i * 0.1}s` }}>
                  ...
                  {ex.note && (
                    <span style={{ display: "inline-block", marginTop: 8, background: "var(--info)", color: "#fff", borderRadius: 20, padding: "2px 10px", fontSize: "var(--text-xs)", fontWeight: 500 }}>
                      {ex.note}
                    </span>
                  )}
                </div>
          ...
          {lesson.table && lesson.table.headers && lesson.table.rows && (
            <div style={{ background: "var(--card)", border: "1px solid var(--card-b)", borderRadius: 14, padding: "20px 20px", overflowX: "auto" }}>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--heading)", marginBottom: 14 }}>📊 Reference Table</div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "var(--text-sm)" }}>
                <thead>
                  <tr>
                    {lesson.table.headers.map((h, i) => (
                      <th key={i} style={{ background: "var(--accent)", color: "#fff", padding: "10px 14px", textAlign: "left", fontWeight: 600, borderRadius: i === 0 ? "8px 0 0 0" : i === lesson.table.headers.length - 1 ? "0 8px 0 0" : 0 }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {lesson.table.rows.map((row, ri) => (
                    <tr key={ri} style={{ background: ri % 2 === 0 ? "var(--bar-bg)" : "transparent" }}>
                      {row.map((cell, ci) => (
                        <td key={ci} style={{ padding: "9px 14px", color: "var(--heading)", borderBottom: "1px solid var(--card-b)" }}>
                          {cell}
                        </td>
                      ))}
                    </tr>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--card-b)", borderRadius: 14, padding: "20px 20px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--heading)" }}>🎯 Quick Quiz</div>
              {quizSubmitted && (
                <span style={{ background: scoreColor, color: "#fff", borderRadius: 20, padding: "4px 14px", fontSize: "var(--text-sm)", fontWeight: 700 }}>
              ...
              {(lesson.quiz || []).map((q, qi) => {
                ...
                return (
                  <div key={qi}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {(q.options || []).map((opt, oi) => {
                        ...
                        if (quizSubmitted && answered === oi && oi !== q.correct) {
                          bg = "#fee2e2"; border = "2px solid #dc2626"; color = "#991b1b";
                        }
                        return (
                          <button
                            key={oi}
                            ...
                            style={{ background: bg, border, borderRadius: 10, padding: "10px 14px", textAlign: "left", cursor: quizSubmitted ? "default" : "pointer", fontSize: "var(--text-sm)", color, display: "flex", alignItems: "center", gap: 8, transition: "all 0.15s" }}
                          >
                    ...
                    {quizSubmitted && (
                      <div style={{ marginTop: 8, fontSize: "var(--text-xs)", color: "var(--subtext)", fontStyle: "italic", padding: "8px 12px", background: "var(--bar-bg)", borderRadius: 8 }}>
                        {q.explanation}
                      </div>
                    )}
                  </div>
            ...
            {!quizSubmitted && (
              <button
                ...
                style={{
                  marginTop: 20, width: "100%", background: quizAnswers.some(a => a === null) ? "var(--bar-bg)" : "var(--accent)",
                  color: quizAnswers.some(a => a === null) ? "var(--subtext)" : "#fff",
                  border: "none", borderRadius: 12, padding: "14px", fontSize: "var(--text-base)", fontWeight: 700, cursor: quizAnswers.some(a => a === null) ? "not-allowed" : "pointer",
                  transition: "all 0.2s",
                }}
              >
                Check Answers
          ...
          {lesson.tip && (
            <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 14, padding: "18px 20px" }}>
              ...
              <p style={{ margin: 0, fontSize: "var(--text-base)", color: "#78350f", lineHeight: 1.6 }}>{lesson.tip}</p>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {selectedTopic && TOPIC_TO_SCREEN[selectedTopic.id] && (
              <button
                ...
                style={{ background: "var(--accent)", color: "#fff", border: "none", borderRadius: 12, padding: "14px", fontSize: "var(--text-base)", fontWeight: 700, cursor: "pointer" }}
              >
                Practice This Topic →
              </button>
            )}
            <button
              onClick={resetToPick}
              style={{ background: "var(--card)", color: "var(--heading)", border: "1px solid var(--card-b)", borderRadius: 12, padding: "14px", fontSize: "var(--text-base)", fontWeight: 600, cursor: "pointer" }}
            >
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--app-bg)", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "16px 16px 0" }}>
        ...
        {error && (
          <div style={{ background: "#fee2e2", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 16px", color: "#991b1b", fontSize: "var(--text-sm)", marginBottom: 16 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--subtext)", marginBottom: 8 }}>Your Level</div>
          ...
          {TOPICS.map(topic => {
            ...
            return (
              <button
                ...
                style={{
                  width: "calc(50% - 6px)", minWidth: 140,
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <span style={{ background: lc.bg, color: lc.text, border: `1px solid ${lc.border}`, borderRadius: 20, padding: "2px 8px", fontSize: "var(--text-xs)", fontWeight: 700 }}>
                    {topic.level}
                  </span>
                </div>
                <div style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--heading)", marginBottom: 2 }}>{topic.title}</div>
                <div style={{ fontSize: "var(--text-xs)", color: "var(--subtext)" }}>{topic.desc}</div>
              </button>
            );
          })}
        </div>
        ...
        >
          {selectedTopic ? `Generate Lesson: ${selectedTopic.title} →` : "Select a topic to continue"}
        </button>
      </div>
    </div>
  );
