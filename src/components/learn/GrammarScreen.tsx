...
const GRAMMAR_ENTRY_TIPS = [
  {
    mood: 'thinking',
    text: 'Read the rule first — understand the *why*, and the practice questions answer themselves. 📐',
  },
  {
    mood: 'ready',
    text: 'Gramatika! Master this one pattern and dozens of sentences unlock instantly. ⚔️',
  },
  {
    mood: 'happy',
    text: 'Croatian grammar is perfectly logical — every ending tells a story. One rule at a time. 🧩',
  },
  {
    mood: 'encouraging',
    text: 'Read each example sentence aloud before clicking Practice. Your ears remember what eyes forget. 🗣️',
  },
];
...
export default function GrammarScreen({
  ...
  useEffect(() => {
    if (gp === 'ex' && gx === 0) {
      knightSpeak(
        'ready',
        'Quiz time! Trust the pattern you just read — your brain already knows the answer. 🎯',
        400,
      );
    } else if (gp === 'result') {
      ...
      knightSpeak(
        ...
        pct === 1
          ? 'Savršeno! Perfect grammar score — this rule is yours forever. 🌟'
          : pct >= 0.7
            ? `Odlično! ${Math.round(pct * 100)}% — solid Croatian grammar. 💪`
        ...
        <React.Fragment>
          ...
          <div className="c" style={{ marginTop: 8 }}>
            ...
            {currentQ.o.map((o, i) => (
              <button
                key={i}
                className={'ob ' + (ga ? (i === currentQ.c ? 'ok' : gsl === i ? 'no' : '') : '')}
                onClick={() => {
                  if (!ga) {
                    sGsl(i);
                    sGa(true);
                    const correct = i === currentQ.c;
                    if (correct) sGs((s) => s + 1);
                    else
                      logError(gl.title || 'grammar_general', 'grammar', {
                        wrong: o,
                        correct: currentQ.o[currentQ.c],
                        source: 'grammar_screen',
                      });
