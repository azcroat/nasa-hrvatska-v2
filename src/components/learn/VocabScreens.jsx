...
const HERO = ({ icon, title, subtitle, color }) => (
  <div style={{ background: `linear-gradient(135deg,${color}dd,${color})`, borderRadius: 18, padding: '20px 20px', marginBottom: 20, color: 'white' }}>
  ...
  <div style={{ display: 'flex', gap: 4, marginBottom: 20, overflowX: 'auto', paddingBottom: 2 }}>
    {tabs.map(t => (
      <button key={t} onClick={() => setActive(t)}
        style={{ flexShrink: 0, padding: '8px 16px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700,
          background: active === t ? accent : '#f5f5f4', color: active === t ? 'white' : '#44403c' }}>
        {t}
      </button>
    ))}
  </div>
...
const TIP_BOX = ({ text }) => (
  <div style={{ background: 'rgba(14,116,144,.06)', borderLeft: '3px solid #0e7490', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: '#44403c', lineHeight: 1.6 }}>
...
const QUIZ_SECTION = ({ quiz, accent }) => {
  ...
  return (
    <div>
      {quiz.map((q, i) => (
        <div key={i} style={{ background: 'white', borderRadius: 14, padding: 16, marginBottom: 12, border: '1px solid rgba(0,0,0,.06)', boxShadow: '0 1px 3px rgba(0,0,0,.04)' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#1c1917', marginBottom: 10 }}>{i + 1}. {q.q}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {q.opts.map((opt, j) => {
              const sel = answers[i];
              const correct = opt === q.a;
              let bg = '#f5f5f4', color = '#44403c', border = '1.5px solid #e7e5e4';
              if (sel !== undefined) {
                if (correct) { bg = '#dcfce7'; color = '#15803d'; border = '1.5px solid #86efac'; }
                else if (sel === j) { bg = '#fee2e2'; color = '#b91c1c'; border = '1.5px solid #fca5a5'; }
              }
              return (
                <button key={j} onClick={() => { if (sel === undefined) setAnswers(a => ({ ...a, [i]: j })); }}
                  style={{ background: bg, color, border, borderRadius: 10, padding: '8px 12px', fontSize: 12, fontWeight: 600, cursor: sel === undefined ? 'pointer' : 'default', textAlign: 'left' }}>
                  {opt}
                </button>
              );
            })}
          ...
          {answers[i] !== undefined && (
            <div style={{ marginTop: 8, fontSize: 11, color: q.opts[answers[i]] === q.a ? '#15803d' : '#b91c1c', fontWeight: 700 }}>
...
export function CountriesScreen({ goBack }) {
  ...
  return (
    <WRAP>
      ...
      <TIP_BOX text={d.tip} />
      <TAB_NAV tabs={['Countries', 'Phrases', 'Quiz']} active={tab} setActive={setTab} accent="#0369a1" />

      {tab === 'Countries' && (
        <div>
          <div style={{ fontSize: 11, color: '#78716c', marginBottom: 12, lineHeight: 1.6 }}>
            Tap the flag to hear the country name. Nationality adjectives: <strong>m / f / n</strong>. Demonyms: person of that nationality.
          </div>
          {d.countries.map((c, i) => (
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
                    {[c.adj_m, c.adj_f, c.adj_n].map((adj, ai) => (
                      <span key={ai} style={{ background: '#f0f9ff', color: '#0369a1', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 8, border: '1px solid #bae6fd' }}>
                        {adj} <span style={{ fontWeight: 400, opacity: 0.7 }}>{['m','f','n'][ai]}</span>
                      </span>
                    ))}
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 11, color: '#44403c' }}>👤 {c.dem_m} / {c.dem_f}</span>
                  </div>
                  {c.note && <div style={{ marginTop: 4, fontSize: 11, color: '#78716c', fontStyle: 'italic' }}>{c.note}</div>}
                </div>
              </div>
            </div>
          ))}
...
export function ProfessionsScreen({ goBack }) {
  ...
  return (
    <WRAP>
      ...
      <TIP_BOX text={d.tip} />
      <TAB_NAV tabs={['Jobs', 'Phrases', 'Quiz']} active={tab} setActive={setTab} accent="#1d4ed8" />
      ...
      {tab === 'Jobs' && (
        <div>
            ...
            {d.categories.map((c, i) => (
              <button key={i} onClick={() => setCatIdx(i)}
                style={{ padding: '6px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700,
                  background: catIdx === i ? '#1d4ed8' : '#f5f5f4', color: catIdx === i ? 'white' : '#44403c' }}>
                {c.icon} {c.cat}
              </button>
            ))}
          </div>
          <div style={{ display: 'grid', gap: 8 }}>
            {cat.jobs.map((j, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 12, padding: '12px 14px', border: '1px solid rgba(0,0,0,.06)', boxShadow: '0 1px 3px rgba(0,0,0,.04)' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {j.note && <div style={{ fontSize: 11, color: '#0369a1', fontStyle: 'italic', marginTop: 3 }}>{j.note}</div>}
                  </div>
                </div>
              </div>
            ))}
...
export function WeatherScreen({ goBack }) {
  ...
  return (
    <WRAP>
      ...
      <TIP_BOX text={d.tip} />
      <TAB_NAV tabs={['Vocabulary', 'Seasons', 'Phrases', 'Quiz']} active={tab} setActive={setTab} accent="#0891b2" />

      {tab === 'Vocabulary' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
            {d.vocab.map((v, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 12, padding: '10px 12px', border: '1px solid rgba(0,0,0,.06)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}
                onClick={() => speak(v.hr)}>
          ...
          <div style={{ fontSize: 13, fontWeight: 800, color: '#164e63', marginBottom: 10 }}>Opposites</div>
          {d.adjectives.map((a, i) => (
            <div key={i} style={{ background: 'white', borderRadius: 10, padding: '10px 14px', marginBottom: 6, border: '1px solid rgba(0,0,0,.06)', display: 'flex', gap: 10, alignItems: 'center' }}>
              <div style={{ flex: 1 }}><span style={{ fontWeight: 700, color: '#0891b2', cursor: 'pointer' }} onClick={() => speak(a.hr)}>{a.hr}</span> <span style={{ color: '#78716c', fontSize: 12 }}>— {a.en}</span></div>
              <div style={{ color: '#d1d5db' }}>⟷</div>
              <div style={{ flex: 1 }}><span style={{ fontWeight: 700, color: '#0891b2', cursor: 'pointer' }} onClick={() => speak(a.hr2)}>{a.hr2}</span> <span style={{ color: '#78716c', fontSize: 12 }}>— {a.en2}</span></div>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === 'Seasons' && (
        <div>
          {d.seasons.map((s, i) => (
            <div key={i} style={{ background: 'white', borderRadius: 14, padding: '16px', marginBottom: 12, border: '1px solid rgba(0,0,0,.06)', boxShadow: '0 1px 3px rgba(0,0,0,.04)', cursor: 'pointer' }}
              onClick={() => speak(s.hr)}>
...
export function ClothesScreen({ goBack }) {
  ...
  return (
    <WRAP>
      ...
      <TIP_BOX text={d.tip} />
      <TAB_NAV tabs={['Clothing', 'Phrases', 'Quiz']} active={tab} setActive={setTab} accent="#7c3aed" />
            ...
            {d.categories.map((c, i) => (
              <button key={i} onClick={() => setCatIdx(i)}
                style={{ padding: '6px 10px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700,
                  background: catIdx === i ? '#7c3aed' : '#f5f5f4', color: catIdx === i ? 'white' : '#44403c' }}>
                {c.icon}
              </button>
            ))}
          </div>
...
export function BodyDescScreen({ goBack }) {
  ...
  return (
    <WRAP>
      ...
      <TIP_BOX text={d.tip} />
      <TAB_NAV tabs={['Description', 'Phrases', 'Quiz']} active={tab} setActive={setTab} accent="#0f766e" />
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
            {d.sections.map((s, i) => (
              <button key={i} onClick={() => setSecIdx(i)}
                style={{ padding: '6px 10px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700,
                  background: secIdx === i ? '#0f766e' : '#f5f5f4', color: secIdx === i ? 'white' : '#44403c', flexShrink: 0 }}>
                {s.icon} {s.title}
              </button>
            ))}
          </div>
...
export function PhonologyScreen({ goBack }) {
  ...
  return (
    <WRAP>
      ...
      <TIP_BOX text={d.tip} />
      <TAB_NAV tabs={['Letters', 'Confusing Pairs', 'Rules', 'Quiz']} active={tab} setActive={setTab} accent="#b45309" />

      {tab === 'Letters' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 16 }}>
            {d.letters.map((l, i) => (
              <button key={i} onClick={() => setSelLetter(selLetter === i ? null : i)}
                style={{ background: selLetter === i ? l.color : l.color + '15', color: selLetter === i ? 'white' : l.color, border: `2px solid ${l.color}30`,
                  borderRadius: 12, padding: '12px 4px', fontSize: 22, fontWeight: 900, cursor: 'pointer', transition: 'all .15s' }}>
                {l.letter}
              </button>
            ))}
          </div>
          {selLetter !== null && (() => {
            ...
            return (
              <div style={{ background: 'white', borderRadius: 16, padding: 20, border: `2px solid ${l.color}30`, boxShadow: '0 4px 16px rgba(0,0,0,.06)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  <div style={{ width: 56, height: 56, borderRadius: 14, background: l.color + '15', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, fontWeight: 900, color: l.color }}>
                    {l.letter}
                  </div>
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 900, color: l.color }}>{l.name}</div>
                    <div style={{ fontSize: 12, color: '#78716c', fontFamily: 'monospace' }}>{l.ipa}</div>
                  </div>
                </div>
                <div style={{ fontSize: 13, color: '#44403c', marginBottom: 10, lineHeight: 1.6 }}>{l.like}</div>
                <div style={{ fontSize: 12, background: l.color + '10', borderRadius: 10, padding: '8px 12px', marginBottom: 12, color: l.color, fontWeight: 700 }}>
                  🧠 {l.memory}
                </div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#164e63', marginBottom: 6 }}>Examples:</div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {l.examples.map((ex, j) => (
                    <div key={j} style={{ background: '#f5f5f4', borderRadius: 10, padding: '6px 12px', cursor: 'pointer' }} onClick={() => speak(ex.hr)}>
          ...
          {d.confusedPairs.map((p, i) => (
            <div key={i} style={{ background: 'white', borderRadius: 14, padding: 16, marginBottom: 12, border: '1px solid rgba(0,0,0,.06)', boxShadow: '0 1px 3px rgba(0,0,0,.04)' }}>
              <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
                <div style={{ flex: 1, textAlign: 'center', background: '#fef3c7', borderRadius: 10, padding: '10px', cursor: 'pointer' }} onClick={() => speak(p.example_a.split(' ')[0])}>
                  <div style={{ fontSize: 26, fontWeight: 900, color: '#b45309' }}>{p.a}</div>
                  <div style={{ fontSize: 11, color: '#44403c', marginTop: 4 }}>{p.example_a}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', color: '#78716c', fontSize: 16 }}>vs</div>
                <div style={{ flex: 1, textAlign: 'center', background: '#fef3c7', borderRadius: 10, padding: '10px', cursor: 'pointer' }} onClick={() => speak(p.example_b.split(' ')[0])}>
                  <div style={{ fontSize: 26, fontWeight: 900, color: '#b45309' }}>{p.b}</div>
                  <div style={{ fontSize: 11, color: '#44403c', marginTop: 4 }}>{p.example_b}</div>
                </div>
              </div>
              <div style={{ fontSize: 12, color: '#44403c', lineHeight: 1.6, background: '#fef9ee', borderRadius: 8, padding: '8px 12px' }}>
                {p.tip}
              </div>
            </div>
          ))}
      ...
      {tab === 'Rules' && (
        <div>
          {d.rules.map((r, i) => (
            <div key={i} style={{ background: 'white', borderRadius: 14, padding: 16, marginBottom: 10, border: '1px solid rgba(0,0,0,.06)', boxShadow: '0 1px 3px rgba(0,0,0,.04)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#b45309', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, flexShrink: 0 }}>{i + 1}</div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#b45309', marginBottom: 4 }}>{r.rule}</div>
