  ...
  function next() {
    if (idx + 1 >= total) {
      if (!finishFired.current) {
        ...
        markQuest('grammar');
        if (!stats.vs?.includes('passive')) {
          setStats((prev) => {
            if (prev.vs?.includes('passive')) return prev;
            return { ...prev, gc: (prev.gc || 0) + 1, vs: [...(prev.vs || []), 'passive'] };
          });
          if (writeDelta) writeDelta({ gc: 1, vs: ['passive'] });
        }
      }
      setDone(true);
    } else {
