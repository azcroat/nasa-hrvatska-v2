...
function buildBank(item) {
  const correctWords = item.hr.replace(/[.?!,;:]/g, ' ').trim().split(/\s+/).filter(Boolean);
  // Pick up to 2 distractor words from wrong option sentences
  const distractors = [];
  for (const opt of (item.opts || [])) {
    if (norm(opt) === norm(item.hr)) continue;
    const words = opt.replace(/[.?!,;:]/g, ' ').trim().split(/\s+/).filter(Boolean);
      ...
      <div className="scr-wrap">
        ...
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <div style={{ fontSize: 16, color: '#64748b', marginBottom: 24 }}>
            {pct >= 80 ? 'Excellent word order!' : pct >= 60 ? 'Good progress!' : 'Keep practising!'}
          </div>
  ...
  return (
    <div className="scr-wrap">
      ...
      <div className="c" style={{ marginBottom: 20, padding: '16px 18px' }}>
        <div style={{ fontSize: 12, color: '#64748b', fontWeight: 700, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.06em' }}>
      ...
      <div style={{
        ...
        borderRadius: 16,
        background: isCorrect ? 'rgba(22,163,74,.06)' : isWrong ? 'rgba(220,38,38,.06)' : 'var(--card)',
      ...
      <div style={{
        ...
        {bank.map(tile => (
          <button
            ...
            onMouseEnter={e => { if (!feedback) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(14,116,144,.2)'; } }}
      ...
      {!feedback ? (
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={() => { setTray([]); setBank(buildBank(q)); }}
            className="b bg"
            style={{ fontSize: 13 }}
          >
            🗑 Clear
