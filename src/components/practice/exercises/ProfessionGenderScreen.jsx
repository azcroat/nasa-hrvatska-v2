        ...
        {[{id:'learn',label:'📖 Learn'},{id:'quiz',label:'🎯 Quiz'}].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, padding: '9px 0', borderRadius: 20, border: 'none', fontWeight: 800, fontSize: 13, cursor: 'pointer',
              background: tab === t.id ? '#0e7490' : 'var(--bar-bg)', color: tab === t.id ? 'white' : 'var(--subtext)', transition: 'all .2s' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'learn' && (
        <>
          <div style={{ marginBottom: 12, padding: '10px 14px', background: 'rgba(14,116,144,.06)', borderRadius: 10, fontSize: 12, color: '#164e63' }}>
            💡 In Croatian, every profession has both a masculine and feminine form. Common patterns: -ač → -ačica, -nik → -nica, -ar → -arka, -ac → -ka.
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, marginBottom: 8, padding: '6px 10px', background: '#0e7490', borderRadius: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: 'rgba(255,255,255,.7)' }}>ENGLISH</div>
            <div style={{ fontSize: 11, fontWeight: 800, color: '#93c5fd', textAlign: 'center' }}>👨 MALE</div>
            <div style={{ fontSize: 11, fontWeight: 800, color: '#f9a8d4', textAlign: 'center' }}>👩 FEMALE</div>
          </div>
          {shMemo('pg', PROFGENDER).map((p, i) => (
            <div key={i} className="c" style={{ marginBottom: 6, padding: '10px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', alignItems: 'center', gap: 8 }}>
              <div style={{ fontSize: 12, color: 'var(--subtext)', fontWeight: 600 }}>{p.en}</div>
              <button style={{ textAlign: 'center', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", padding: 0 }}
                onClick={() => speak(p.m)}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#1e40af' }}>{'👨 '}{p.m}</div>
              </button>
              <button style={{ textAlign: 'center', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", padding: 0 }}
                onClick={() => speak(p.f)}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#db2777' }}>{'👩 '}{p.f}</div>
              </button>
            </div>
          ))}
        </>
        ...
        <>
            ...
            return (
              <div key={qi} className="c" style={{ marginBottom: 10, padding: '12px 14px' }}>
                <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: '#1c1917' }}>
                  {q.gender === 'm' ? '👨' : '👩'} {q.gender === 'm' ? 'Male' : 'Female'} form of: <span style={{ color: '#0e7490' }}>{q.en}</span>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                  {q.opts.map((opt, oi) => {
                    ...
                    if (chosen !== undefined) {
                      ...
                      else if (!chosen && oi === q.opts.indexOf(opt)) { bg = '#fee2e2'; bc = '#dc2626'; col = '#7f1d1d'; }
                    }
                    return (
                      <button key={oi}
                        onClick={() => handleAnswer(qi, opt, q.a)}
                        style={{ padding: '7px 16px', border: `2px solid ${bc}`, borderRadius: 10, background: bg, color: col, fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'all .15s' }}>
                        {opt}
                      </button>
                    );
                  })}
          ...
          {allDone && (
            <div className="c" style={{ marginTop: 16, padding: '20px 16px', textAlign: 'center' }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}>{correctCount / quiz.length >= 0.8 ? '🏆' : correctCount / quiz.length >= 0.6 ? '⭐' : '💪'}</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: '#164e63', marginBottom: 4 }}>{correctCount}/{quiz.length} correct</div>
