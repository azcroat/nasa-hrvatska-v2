...
function buildQuiz(verbs) {
  ...
  pool.forEach((verb, i) => {
    ...
    qs.push({ verb: verb.inf, en: verb.en, person: VBPERSONS[personIdx], correct, opts: sh([correct, ...distractors]) });
  });
  return qs;
}
  ...
  if (mode === 'reference') {
    return (
      <div className="scr-wrap">
        ...
        {verbs.map(function(v,vi){return (
          <div key={vi} className="c" style={{marginBottom:10,padding:0,overflow:"hidden"}}>
            <div style={{padding:"8px 14px",background:"linear-gradient(135deg,#0e7490,#164e63)",color:"white",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={function(){speak(v.inf)}}>
              ...
              <span style={{fontSize:12,opacity:.7}}>{v.en} <span aria-hidden="true">🔊</span></span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}}>
              {v.forms.map(function(f,fi){return (
                <button key={fi} style={{padding:"6px 12px",fontSize:12,cursor:"pointer",display:"flex",gap:6,background:"none",border:"none",borderBottom:"1px solid #f0fdfa",borderRight:fi%2===0?"1px solid #f0fdfa":"none",fontFamily:"'Outfit',sans-serif",textAlign:"left"}} onClick={function(){speak(f)}}>
      ...
      <div className="scr-wrap">
        ...
        <div style={{textAlign:"center",padding:"32px 16px"}}>
          ...
          <div style={{fontSize:13,color:"#78716c",marginBottom:16}}>
            {pct>=0.9?"Verb master! Your conjugation is solid.":pct>=0.7?"Great progress! Review the ones you missed.":"Keep drilling — verb forms click with repetition."}
          </div>
          <div style={{fontSize:22,fontWeight:900,color:"#d97706",marginBottom:24}}>+{score*3+10} XP</div>
  ...
  return (
    <div className="scr-wrap">
      ...
      <div className="c" style={{marginBottom:16,padding:"18px 16px",textAlign:"center"}}>
        <div style={{fontSize:12,color:"#78716c",marginBottom:6,fontWeight:700,letterSpacing:".05em"}}>CONJUGATE</div>
        <div style={{fontSize:22,fontWeight:900,color:"#164e63",marginBottom:4}}>{q.verb}</div>
        <div style={{fontSize:13,color:"#78716c",marginBottom:10}}>{q.en}</div>
        <div style={{display:"inline-block",background:"#0e7490",color:"white",fontSize:15,fontWeight:800,padding:"6px 18px",borderRadius:20}}>
          {q.person} + ?
        </div>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {q.opts.map((o,oi) => {
          let bg = "white", bc = "#e7e5e4", col = "#1c1917", fw = 600;
          if (answered) {
            if (o === q.correct) { bg = "#dcfce7"; bc = "#16a34a"; col = "#14532d"; fw = 800; }
          ...
          return (
            <button key={oi} onClick={() => handleAnswer(o)}
              style={{padding:"14px 16px",border:`2px solid ${bc}`,borderRadius:12,background:bg,
                color:col,fontSize:16,fontWeight:fw,cursor:answered?"default":"pointer",
