...
function GenderDrillScreen({ goBack, award, setSt }: Props) {
  ...
  const gColor = {
    m: { bg: '#dbeafe', bc: '#1e40af', tc: '#1e40af' },
    f: { bg: '#fce7f3', bc: '#db2777', tc: '#db2777' },
    n: { bg: '#dcfce7', bc: '#16a34a', tc: '#16a34a' },
  };
  ...
  return (
    <div className="scr-wrap">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, fontSize: 12, color: '#78716c' }}>
        ...
        >
          Sort {Object.keys(revealedGenders).length}/{words.length}
        </span>
        <span
          style={{
        ...
        >
          Plural {Object.keys(pluralAnswered).length}/{plurals.length}
        </span>
        <span
          style={{
        ...
        >
          Adjective {Object.keys(adjAnswered).length}/{GENDERDRILL.adjectives.length}
        </span>
      </div>
            ...
            {(['m', 'f', 'n'] as const).map((g) => (
              <button
                ...
                onClick={() => {
                  ...
                  setRevealedGenders((prev) => ({
                    ...prev,
                    [selectedGenderIdx]: { guess: g, correct },
                  }));
              ...
              {(pluralOpts[i] ?? []).map((opt, oi) => {
                ...
                if (answered) {
                  if (opt === p.p) {
                    bg = '#dcfce7';
                    bc = '#16a34a';
                  } else if (opt === answered.guess && !answered.correct) {
                    bg = '#fee2e2';
                    bc = '#dc2626';
                  }
                }
                return (
                  ...
                  >
                    {answered && opt === p.p
                      ? '✓ '
                      : answered && opt === answered.guess && !answered.correct
                        ? '✗ '
                        : ''}
                    {opt}
                  </button>
              ...
              {a.opts.map((o, oi) => {
                ...
                if (answered) {
                  if (o === a.adj) {
                    bg = '#dcfce7';
                    bc = '#16a34a';
                  } else if (o === answered.guess && !answered.correct) {
                    bg = '#fee2e2';
                    bc = '#dc2626';
                  }
                }
                return (
                  ...
                  >
                    {answered && o === a.adj
                      ? '✓ '
                      : answered && o === answered.guess && !answered.correct
                        ? '✗ '
                        : ''}
                    {o}
                  </button>
