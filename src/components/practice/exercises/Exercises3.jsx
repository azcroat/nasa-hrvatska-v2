          ...
          <div style={{display:"flex",gap:4}}>
            {q.opts.map(function(o,oi){return (
              <button key={oi} style={{padding:"8px 14px",border:"2px solid #d6d3d1",borderRadius:10,background:"white",fontSize:11,cursor:"pointer"}}
                onClick={function(/** @type {any} */ e){e.target.style.background=o===q.color?"#dcfce7":"#fee2e2";e.target.style.borderColor=o===q.color?"#16a34a":"#dc2626";if(o===q.color){if(typeof award==='function')award(3);speak(q.noun+" je "+q.color);}e.target.closest&&e.target.closest("div")&&(e.target.closest("div").style.pointerEvents="none")}}>
                {o}
              </button>
            );})}
          </div>
      ...
      {shMemo("cp",COLORAGREE.plurQuiz).map(function(q,qi){return (
        <div key={qi} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:4}}>
            {q.opts.map(function(o,oi){return (
              <button key={oi} style={{padding:"8px 14px",border:"2px solid #d6d3d1",borderRadius:10,background:"white",fontSize:11,cursor:"pointer"}}
                onClick={function(/** @type {any} */ e){e.target.style.background=o===q.color?"#dcfce7":"#fee2e2";e.target.style.borderColor=o===q.color?"#16a34a":"#dc2626";if(o===q.color){if(typeof award==='function')award(3);speak(q.noun+" su "+q.color);}e.target.closest&&e.target.closest("div")&&(e.target.closest("div").style.pointerEvents="none")}}>
                {o}
              </button>
            );})}
          </div>
      ...
      {shMemo("pq",POSSESS.quiz,10).map(function(q,qi){return (
        <div key={qi} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:4}}>
            {q.opts.map(function(o,oi){return (
              <button key={oi} style={{padding:"8px 14px",border:"2px solid #d6d3d1",borderRadius:10,background:"white",fontSize:11,cursor:"pointer"}}
                onClick={function(/** @type {any} */ e){e.target.style.background=o===q.a?"#dcfce7":"#fee2e2";e.target.style.borderColor=o===q.a?"#16a34a":"#dc2626";if(o===q.a){if(typeof award==='function')award(3);speak("Ovo je "+q.a+" "+q.noun);}e.target.closest&&e.target.closest("div")&&(e.target.closest("div").style.pointerEvents="none")}}>
                {o}
              </button>
            );})}
          </div>
...
export function QuestionWordsScreen({ goBack, award }) {
  ...
  const [answers, setAnswers] = useState(() => new Array(total).fill(null));   // null | 'correct' | 'wrong'
  ...
  return (
    <div className="scr-wrap">
      ...
      {answeredCount > 0 && !allDone && (
        <div style={{marginBottom:12,background:"rgba(0,0,0,.06)",borderRadius:8,height:5,overflow:"hidden"}}>
          <div style={{width:`${(answeredCount/total)*100}%`,height:"100%",background:"linear-gradient(90deg,#0e7490,#06b6d4)",borderRadius:8,transition:"width .3s"}}/>
        </div>
      )}
            ...
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              {q.opts.map(function(o, oi) {
                ...
                if (state !== null) {
                  ...
                  else if (o === sel) { bg = "#fee2e2"; border = "#dc2626"; }
                }
                return (
                  <button key={oi}
                    disabled={state !== null}
                    style={{padding:"6px 14px",border:`2px solid ${border}`,borderRadius:10,background:bg,fontSize:12,fontWeight:600,cursor:state!==null?"default":"pointer",transition:"background .15s,border-color .15s"}}
              ...
              {state === 'wrong' && <span style={{fontSize:13,color:"#0e7490",fontWeight:700}}>→ {q.a}</span>}
            </div>
          </div>
        );
      })}
      ...
      {/* Completion card */}
      {allDone && (
        <div className="c" style={{marginTop:16,padding:"20px 16px",background:"linear-gradient(135deg,rgba(14,116,144,.09),rgba(6,182,212,.06))",textAlign:"center",border:"1.5px solid rgba(14,116,144,.22)"}}>
          ...
          <button onClick={goBack} style={{padding:"10px 28px",background:"linear-gradient(135deg,#0e7490,#0891b2)",color:"white",border:"none",borderRadius:12,fontWeight:700,fontSize:14,cursor:"pointer"}}>
            Done
          </button>
        </div>
      )}
      ...
      {shMemo("cq",COMPQUIZ).map(function(q,qi){return (
        <div key={qi} className="c" style={{marginBottom:8,padding:"10px 14px"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {q.opts.map(function(o,oi){return (
              <button key={oi} style={{padding:"6px 14px",border:"2px solid #d6d3d1",borderRadius:10,background:"white",fontSize:12,fontWeight:600,cursor:"pointer"}}
                onClick={function(/** @type {any} */ e){e.target.style.background=o===q.a?"#dcfce7":"#fee2e2";e.target.style.borderColor=o===q.a?"#16a34a":"#dc2626";if(o===q.a){if(typeof award==='function')award(3);}e.target.closest&&e.target.closest("div")&&(e.target.closest("div").style.pointerEvents="none")}}>{o}</button>
            );})}
          </div>
        </div>
      );})}
