...
function CityLocativeScreen({ goBack, award }) {
  ...
  return (
    <div className="scr-wrap">
      ...
      {quizCities.map(function(c2,i){const wrong=CITYLOC.cities[(i+3)%CITYLOC.cities.length].lok;return (
          ...
          <div style={{display:"flex",gap:4}}>
            {sh([c2.lok,wrong]).map(function(o,oi){return (
              <button key={oi} style={{padding:"8px 14px",border:"2px solid #d6d3d1",borderRadius:10,background:"white",fontSize:11,fontWeight:600,cursor:"pointer"}}
                onClick={function(e){handleAnswer(e, o===c2.lok);}}>{o}</button>
            );})}
          </div>
        </div>
      );})}
      {done && (
        <div className="c" style={{marginTop:16,padding:"20px 16px",textAlign:"center"}}>
          <div style={{fontSize:40,marginBottom:8}}>{correctRef.current/quizCities.length>=0.8?"🏆":correctRef.current/quizCities.length>=0.6?"⭐":"💪"}</div>
          <div style={{fontSize:18,fontWeight:800,color:"#164e63",marginBottom:4}}>{correctRef.current}/{quizCities.length} correct</div>
