  ...
  return (
    <div style={{maxWidth:620,margin:"0 auto",padding:"24px 16px",paddingBottom:80,position:"relative",zIndex:1}}>
      ...
      <div className="c" style={{marginBottom:16,padding:"12px 16px",background:"rgba(14,116,144,.06)"}}>
        <div style={{fontSize:13,color:"#164e63"}}>{REFLEXIVE.intro}</div>
      </div>
      ...
      <h3 className="sh" style={{marginTop:20}}>🎯 Quick Quiz</h3>
      <div className="c">
        <div style={{fontSize:14,fontWeight:700,marginBottom:8}}>{"🇬🇧 "}{q.q}</div>
        {q.opts.map(function(o,oi){return (
          <button key={oi} className="ob" style={{display:"block",width:"100%",marginBottom:6,textAlign:"left"}}
      ...
      {GENDERDRILL.plurals.slice(0,10).map(function(p,i){return (
        <div key={i} className="c" style={{marginBottom:8,padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button style={{padding:"6px 12px",border:"2px solid #d6d3d1",borderRadius:10,background:"white",fontSize:12,cursor:"pointer"}}
            onClick={function(e){e.target.innerHTML=p.p;e.target.style.background="#dcfce7";e.target.style.borderColor="#16a34a";speak(p.p);award(2);e.target.closest&&e.target.closest("div")&&(e.target.closest("div").style.pointerEvents="none")}}>Show</button>
        </div>
      );})}
