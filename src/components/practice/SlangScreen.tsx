...
export default function SlangScreen({
  ...
  const [visitedSections, setVisitedSections] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('slangVisited') || '[]');
    } catch {
      return [];
    }
  });
  ...
  function startQuiz(sec: { entries: { hr: string; en: string; ph: string }[] }) {
    ...
    const pool = sh(sec.entries.filter((e) => e.en && e.en.length < 60 && e.ph !== '—')).slice(
      0,
      Math.min(6, sec.entries.length),
    );
    if (pool.length < 2) return;
    const qs = pool.map((entry) => {
      const wrong = sh(allEntries.filter((e) => e !== entry && e.en && e.en.length < 60)).slice(
        0,
        3,
      );
          ...
          s.entries
            .filter(
              (e) =>
                ...
                e.variants.some(
                  (v) => v.hr.toLowerCase().includes(q) || v.en.toLowerCase().includes(q),
                ),
            )
            .map((e) => ({
              ...e,
              sectionColor: s.color,
              sectionLight: s.light,
              sectionBorder: s.border,
              sectionIcon: s.icon,
    ...
    <div className="scr-wrap" style={{ paddingBottom: 100 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "'Playfair Display',serif" }}>
              Slang & Psovanje
            </div>
            <div style={{ fontSize: 11, opacity: 0.65, marginTop: 2 }}>
              12 sections · 150+ expressions · all with pronunciation
            </div>
          </div>
        </div>
        {/* Search */}
      ...
      {!searching && (
        <>
          ...
          >
            ...
            Croatian swearing is deeply social and tone-dependent. The same phrase can be a
            declaration of love between friends or a serious insult between strangers. Tone,
            relationship, and setting determine meaning entirely. When in doubt, listen first.
          </div>
        </>
      )}
