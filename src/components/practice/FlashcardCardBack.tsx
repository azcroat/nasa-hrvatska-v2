...
function getWordTip(croatianWord: string, englishMeaning: string) {
  ...
  if (isVerb) {
    ...
    return {
      type: 'verb',
      hr: `${w} → ${stem}im / ${stem}iš`,
      en: 'infinitive → I / you (present)',
    };
  }
  ...
  return (
    <>
      ...
      {exampleSentence ? (
        ...
        (() => {
          const genderColor =
            tip.gender === 'm.' ? '#3b82f6' : tip.gender === 'f.' ? '#ec4899' : '#a855f7';
          return (
      ...
      {!exampleSentence && (
        ...
        >
          ...
          {!aiLoading && aiSentence && (
            <>
              <div
                style={{ fontSize: 14, fontWeight: 700, color: 'var(--heading)', lineHeight: 1.5 }}
              >
                {aiSentence.hr}
              </div>
              <div
                style={{ fontSize: 12, color: 'var(--subtext)', marginTop: 3, fontStyle: 'italic' }}
              >
                {aiSentence.en}
              </div>
              {aiSentence.note && (
