...
export default function McGame({ questions: rawQuestions, onComplete, goBack, award, challengeMode = false }) {
  ...
  function handleAnswer(o, i) {
    ...
    if (isCorrect) {
      ...
      apiFetch('/api/explain-error', {
        ...
        body: JSON.stringify({ wrong: wrongOpt, correct: q.correct, context: q.hr || q.q || '', type: 'multiple_choice', level: localStorage.getItem('nh_level') || 'B1' }),
    ...
    if (isCorrect) {
      ...
      const newHearts = isHeartsMode
        ...
        : (state.practiceMode ? state.hearts : Math.max(0, state.hearts - 1));
      if (newHearts === 0) {
  ...
  const wasCurrentCorrect = state.answered && state.selected !== -1 && q.opts[state.selected] === q.correct;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        {state.confirmQuit ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
            ...
            <button onClick={() => dispatch({ type: 'SET_CONFIRM_QUIT', value: false })} style={{ padding: '12px 20px', borderRadius: 10, border: '1.5px solid var(--inp-b)', background: 'none', color: 'var(--subtext)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'Outfit',sans-serif", minHeight: 44 }}>Keep going</button>
          </div>
        ) : (
          <button
            onClick={() => {
              if (state.clearedCount === 0 && !state.answered) { goBack(); }
              else { dispatch({ type: 'SET_CONFIRM_QUIT', value: true }); }
            }}
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 24, color: 'var(--subtext)', padding: '10px 16px', minHeight: 44, minWidth: 44, borderRadius: 10, transition: 'background .15s' }}
            aria-label="Go back"
          >
            ×
          </button>
        ...
        <div style={{ flex: 1, margin: '0 12px', position: 'relative' }}>
          <div style={{ height: 10, background: 'var(--bar-bg)', borderRadius: 10, overflow: 'hidden' }}>
            <div
              style={{
                height: '100%', width: '100%',
                background: 'linear-gradient(90deg,var(--info),#06b6d4)',
                borderRadius: 10, transformOrigin: 'left center',
                transform: `scaleX(${progress / 100})`,
                transition: 'transform 0.5s cubic-bezier(0.4,0,0.2,1)',
                position: 'relative', overflow: 'hidden',
              }}
            >
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg,transparent,rgba(255,255,255,.35),transparent)', animation: 'shimmer 1.5s infinite', backgroundSize: '200% 100%' }} />
            </div>
          </div>
        </div>
        ...
        {state.streak >= 1 && (
          <div
            style={{
              ...
              border: state.streakPulse ? '1.5px solid rgba(249,115,22,.7)' : '1.5px solid rgba(249,115,22,.3)',
              borderRadius: 20, padding: '4px 10px', fontSize: 13, fontWeight: 800,
          ...
          >
            <span style={{ animation: 'flameDance 1s ease-in-out infinite', display: 'inline-block' }}>🔥</span>
            {state.streak}
          </div>
        )}
        ...
        <button
          ...
          aria-label={state.practiceMode ? 'Practice Mode active — click to disable' : 'Enable Practice Mode'}
          style={{
            background: state.practiceMode ? 'rgba(99,102,241,0.15)' : 'none',
            border: state.practiceMode ? '1.5px solid rgba(99,102,241,0.5)' : '1.5px solid transparent',
            borderRadius: 10, cursor: 'pointer', fontSize: 20, padding: '6px 10px',
            minHeight: 44, minWidth: 44, marginLeft: 6, transition: 'background .15s, border-color .15s',
          }}
        >
          🛡️
        </button>
      ...
      {/* Score line */}
      <div style={{ textAlign: 'center', fontSize: 12, fontWeight: 700, color: 'var(--subtext)', marginBottom: 16, letterSpacing: '.05em' }}>
        {q._isRetry
      ...
      {/* "On a roll" banner */}
      {state.showOnARoll && (
        <div style={{ textAlign: 'center', fontSize: 15, fontWeight: 900, color: '#f59e0b', animation: 'bounce-in 0.4s cubic-bezier(0.34,1.56,0.64,1)', marginBottom: 10, letterSpacing: '0.02em' }}>
          🔥 You're on a roll!
        </div>
      )}
      ...
      {state.wrongStreak >= 3 && (
        <div style={{ marginBottom: 10, padding: '8px 14px', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 10, fontSize: 12, color: '#92400e', fontWeight: 600, textAlign: 'center' }}>
      ...
      {state.grammarTip && state.answered && !wasCurrentCorrect && (
        <div style={{ marginBottom: 10, padding: '10px 14px', background: 'rgba(99,102,241,0.08)', border: '1.5px solid rgba(99,102,241,0.25)', borderRadius: 12, fontSize: 12, color: '#3730a3', fontWeight: 600, lineHeight: 1.5, animation: 'bounce-in 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}>
          {state.grammarTip}
        </div>
      )}
      ...
      {state.answered && !wasCurrentCorrect && aiExplain === 'loading' && (
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 14px', borderRadius:12, marginBottom:10, background:'var(--bar-bg)', border:'1px solid var(--card-b)', fontSize:12, color:'var(--subtext)', fontWeight:600, animation:'fadeIn .3s ease' }}>
          <span style={{ animation:'pulse 1.5s ease-in-out infinite' }}>🤖</span>
          <span>Getting AI explanation…</span>
        </div>
      )}
      {state.answered && !wasCurrentCorrect && aiExplain && aiExplain !== 'loading' && (
        <div style={{ background:'linear-gradient(135deg,rgba(124,58,237,.06),rgba(124,58,237,.03))', border:'1.5px solid rgba(124,58,237,.22)', borderRadius:14, padding:'14px 16px', marginBottom:10, animation:'spring-in .3s ease .1s both' }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:8, fontSize:11, fontWeight:800, color:'#7c3aed', textTransform:'uppercase', letterSpacing:'.08em' }}>
            <span>🤖</span><span>AI Explanation · {aiExplain.rule}</span>
          </div>
          <div style={{ fontSize:13, color:'var(--heading)', lineHeight:1.6, marginBottom: aiExplain.tip ? 8 : 0 }}>
            {aiExplain.explanation}
          </div>
          {aiExplain.tip && (
            <div style={{ fontSize:12, color:'var(--subtext)', fontStyle:'italic', padding:'6px 10px', borderRadius:8, marginTop:4, background:'rgba(124,58,237,.06)', borderLeft:'3px solid rgba(124,58,237,.4)' }}>
              💡 {aiExplain.tip}
            </div>
          )}
          {aiExplain.example && (
