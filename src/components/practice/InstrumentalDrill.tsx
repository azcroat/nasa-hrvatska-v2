  ...
  function next() {
    if (idx + 1 >= total) {
      if (!finishFired.current) {
        ...
        markQuest('grammar');
        if (!stats.vs?.includes('instrumental')) {
          setStats((prev) => {
            if (prev.vs?.includes('instrumental')) return prev;
            return { ...prev, gc: (prev.gc || 0) + 1, vs: [...(prev.vs || []), 'instrumental'] };
          });
          if (writeDelta) writeDelta({ gc: 1, vs: ['instrumental'] });
        }
      }
      setDone(true);
    } else {
