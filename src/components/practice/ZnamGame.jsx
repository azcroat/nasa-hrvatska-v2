...
export default function ZnamGame({ goBack, award }) {
  ...
  function startSection(si) {
    ...
    sZnSec(si); sZnIdx(0); sZnSc(0); sZnAns(false); sZnSel(-1);
    sZnOpts(sh([s0.hr, ...s0.alts])); sZnMode("quiz");
  }
        ...
        <React.Fragment>
          ...
          {ZNAM.sections.map((sec, si) => (
            <div
              ...
              <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#0e7490,#06b6d4)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:800,fontSize:16}}>
                {si + 1}
              </div>
              <div>
                <div style={{fontSize:15,fontWeight:700}}>{sec.name}</div>
                <div style={{fontSize:12,color:"#78716c"}}>{sec.sentences.length} sentences</div>
              </div>
            </div>
          ))}
        </React.Fragment>
        ...
        return (
          <React.Fragment>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              <div style={{fontSize:14,fontWeight:700}}>{sec.name} — {znIdx + 1}/{sec.sentences.length}</div>
              <div style={{fontSize:14,fontWeight:700,color:"#0e7490"}}>Score: {znSc}</div>
            </div>
            <Bar v={znIdx + 1} mx={sec.sentences.length} h={6} />
            <div className="c" style={{marginTop:16,textAlign:"center"}}>
              <div style={{fontSize:12,color:"#78716c",marginBottom:6}}>Translate to Croatian:</div>
              ...
              <button
                ...
                onClick={() => {
                  if (znIdx < sec.sentences.length - 1) {
                    ...
                    sZnIdx(i => i + 1); sZnAns(false); sZnSel(-1); sZnOpts(sh([nxt.hr, ...nxt.alts]));
                  } else {
                    sZnMode("done");
                  }
                }}>
          ...
          <div style={{textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
              ...
              {znSec < ZNAM.sections.length - 1 && (
                <button className="b bp" onClick={() => startSection(znSec + 1)}>Next Section →</button>
              )}
              <button className="b bg" onClick={() => sZnMode("menu")}>📋 All Sections</button>
            </div>
          </div>
        );
      })()}
