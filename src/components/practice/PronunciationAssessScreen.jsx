...
const PHRASES = {
  ...
  B1: [
    { hr: 'Hrvatska ima lijepu obalu i bogatu kulturu.', en: 'Croatia has a beautiful coastline and rich culture.' },
    ...
    { hr: 'Ljubav je najvažnija stvar na svijetu.', en: 'Love is the most important thing in the world.' },
    { hr: 'Šuma je puna divljih životinja.', en: 'The forest is full of wild animals.' },
    { hr: 'Nikola Tesla je bio genijalan izumitelj.', en: 'Nikola Tesla was a brilliant inventor.' },
    { hr: 'Đak uči novi jezik svaki dan.', en: 'The student learns a new language every day.' },
    { hr: 'Trebali bismo čuvati prirodu za buduće generacije.', en: 'We should preserve nature for future generations.' },
  ],
  B2: [
    { hr: 'Glagolski vid je jedna od najtežih pojava u hrvatskom.', en: 'Verbal aspect is one of the most difficult phenomena in Croatian.' },
    { hr: 'Dalmatinska obala privlači milijune turista svake godine.', en: 'The Dalmatian coast attracts millions of tourists every year.' },
    { hr: 'Uvođenje eura značajno je promijenilo svakodnevni život.', en: 'The introduction of the euro significantly changed everyday life.' },
    { hr: 'Knjiga koju sam pročitao prošlog tjedna bila je iznimno zanimljiva.', en: 'The book I read last week was exceptionally interesting.' },
    { hr: 'Bez obzira na poteškoće, nastavio je učiti hrvatski.', en: 'Despite the difficulties, he continued learning Croatian.' },
    { hr: 'Njezina ljubaznost i strpljenje ostavili su dubok dojam.', en: 'Her kindness and patience left a deep impression.' },
    { hr: 'Kada bih imao više vremena, više bih putovao po Balkanu.', en: 'If I had more time, I would travel more around the Balkans.' },
    { hr: 'Hrvatska povijest isprepletena je s europskim zbivanjima od davnina.', en: 'Croatian history is intertwined with European events from ancient times.' },
    { hr: 'Klapa je tradicijska vrsta višeglasnog pjevanja iz Dalmacije.', en: 'Klapa is a traditional form of polyphonic singing from Dalmatia.' },
    { hr: 'Stjepan Radić je bio jedan od najvažnijih hrvatskih političara dvadesetog stoljeća.', en: 'Stjepan Radić was one of the most important Croatian politicians of the twentieth century.' },
  ],
};
...
function overallGrade(avg) {
  if (avg >= 90) return { letter: 'A', label: 'Excellent', color: '#16a34a', advice: 'Outstanding Croatian pronunciation! Your sounds are clear and accurate. Keep challenging yourself with longer, more complex sentences.' };
  if (avg >= 80) return { letter: 'B', label: 'Good', color: '#0284c7', advice: 'Strong pronunciation with good clarity. Focus on the sounds that scored yellow to reach native-level accuracy.' };
  if (avg >= 70) return { letter: 'C', label: 'Developing', color: '#d97706', advice: 'Good foundation — keep practising the phoneme drills, especially for ć/č, š/ž, and the rolled \'r\'.' };
  if (avg >= 55) return { letter: 'D', label: 'Needs Work', color: '#ea580c', advice: 'Regular pronunciation practice will make a big difference. Try the Phoneme Course to master each sound individually.' };
  return { letter: 'F', label: 'Beginner', color: '#dc2626', advice: 'Every expert started here. Focus on the Phoneme Course first to build the sounds, then return for a re-test.' };
}
...
function ScoreChip({ score }) {
  ...
  return (
    <span style={{
      ...
      fontSize: 13, fontWeight: 700, color: c, fontVariantNumeric: 'tabular-nums',
    }}>
...
export default function PronunciationAssessScreen({ goBack, award }) {
  ...
  const handleScore = useCallback((result) => {
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px 100px' }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px 0 12px', gap: 8 }}>
          <button onClick={goBack} style={{ background: 'none', border: 'none', color: 'var(--heading)', fontSize: 16, fontWeight: 600, cursor: 'pointer', padding: '4px 0' }}>← Natrag</button>
        </div>
        ...
        <div style={{ textAlign: 'center', padding: '32px 0 24px' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--subtext)', lineHeight: 1.6, marginBottom: 0 }}>
            Speak 8 Croatian phrases and receive a personalised pronunciation score.
            Powered by Azure Speech recognition for hr-HR Croatian.
          </p>
        </div>
        ...
        <div style={{
          background: 'rgba(212,0,45,0.06)', border: '1px solid rgba(212,0,45,0.2)',
          borderRadius: 12, padding: '14px 16px', marginBottom: 24,
          fontSize: 13, color: 'var(--subtext)', lineHeight: 1.5,
        }}>
          <strong style={{ color: 'var(--heading)' }}>💡 Tips:</strong> Find a quiet space · Speak clearly and at normal pace · The first attempt always counts most · Chrome / Edge give the most accurate results
        </div>
  ...
  if (isResults) {
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px 100px' }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px 0 12px', gap: 8 }}>
          <button onClick={goBack} style={{ background: 'none', border: 'none', color: 'var(--heading)', fontSize: 16, fontWeight: 600, cursor: 'pointer', padding: '4px 0' }}>← Done</button>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: `${grade.color}20`, border: `2px solid ${grade.color}50`,
            fontSize: 32, fontWeight: 900, color: grade.color, marginBottom: 12,
          }}>
            {grade.letter}
          </div>
          ...
          {phrases.map((p, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--subtext)', width: 20, flexShrink: 0 }}>
                  {i + 1}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--heading)', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {p.hr}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--subtext)' }}>{p.en}</div>
                </div>
                {s !== undefined ? <ScoreChip score={s} /> : (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button
            onClick={() => setScr('pronunciation_course')}
            style={{
              padding: '14px', borderRadius: 12, border: '1.5px solid rgba(212,0,45,0.4)',
              background: 'rgba(212,0,45,0.06)', color: '#D4002D',
          ...
          <button
            ...
            style={{
              padding: '14px', borderRadius: 12, border: '1.5px solid var(--card-b)',
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px 100px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0 12px' }}>
        <button onClick={goBack} style={{ background: 'none', border: 'none', color: 'var(--heading)', fontSize: 16, fontWeight: 600, cursor: 'pointer', padding: '4px 0' }}>← Cancel</button>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--subtext)', textTransform: 'uppercase', marginBottom: 10 }}>
          Say this in Croatian
        </div>
        <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--heading)', lineHeight: 1.3, marginBottom: 8 }}>
          {currentPhrase.hr}
        </div>
