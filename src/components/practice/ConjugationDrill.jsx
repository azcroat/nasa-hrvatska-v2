...
export default function ConjugationDrill({ goBack, award, setSt }) {
  ...
  function startQuiz(tense) {
    ...
    const wrongs = sh(CONJ.verbs.flatMap(v => v.forms).filter(f => f !== first.answer)).slice(0, 3);
    sCjQ(picked); sCjI(0); sCjS(0); sCjA(false); sCjSl(-1); sCjO(sh([first.answer].concat(wrongs))); sCjMode("quiz");
  }
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {["all","present","past","future"].map(t => (
              <div
                ...
                <div style={{fontSize:15,fontWeight:700,marginTop:6,textTransform:"capitalize"}}>
                  {t === "all" ? "All Tenses" : t + " Tense"}
                </div>
                <div style={{fontSize:12,color:"#78716c"}}>
                  {t === "all" ? CONJ.verbs.length + " verbs" : CONJ.verbs.filter(v => v.tense === t).length + " verbs"}
                </div>
              </div>
            ))}
          </div>
          <div className="c" style={{marginTop:20}}>
            <div style={{fontSize:14,fontWeight:700,color:"#0e7490",marginBottom:10}}>📖 Verb Reference</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
              {CONJ.verbs.filter(v => v.tense === "present").map((v, i) => (
                <div
                  key={i}
        ...
        if (cjI >= total) {
          ...
          return (
            <div style={{textAlign:"center"}}>
              ...
              <h2 style={{fontFamily:"'Playfair Display',serif",color:"#164e63"}}>Conjugation Complete!</h2>
        ...
        return (
          <React.Fragment>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              <div style={{fontSize:14,fontWeight:700}}>{cjI + 1} / {total}</div>
              <div style={{fontSize:14,fontWeight:700,color:"#0e7490"}}>Score: {cjS}</div>
            </div>
            <Bar v={cjI + 1} mx={total} color={tC} h={6} />
            <div className="c" style={{marginTop:16,textAlign:"center"}}>
              <div style={{display:"inline-block",padding:"3px 12px",borderRadius:14,fontSize:11,fontWeight:800,color:"#fff",background:tC,marginBottom:8}}>{tL}</div>
              <div style={{fontSize:22,fontWeight:800,color:"#164e63"}}>{q.verb} ({q.en})</div>
              <div style={{fontSize:18,fontWeight:700,color:tC,marginTop:8}}>{q.person} ___?</div>
            </div>
              ...
              <button
                ...
                onClick={() => {
                  if (cjI < total - 1) {
                    ...
                    const wrongs = sh(CONJ.verbs.flatMap(v => v.forms).filter(f => f !== next.answer)).slice(0, 3);
