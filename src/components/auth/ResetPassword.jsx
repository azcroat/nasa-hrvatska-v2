...
export default function ResetPassword({
  authError, rpEm, rpSa, rpPw, rpPc, rpStep, rpQ,
  setAuthScreen, setAuthError, setRpEm, setRpSa, setRpPw, setRpPc, setRpStep, setRpQ,
  doReset,
}) {
  return (
      ...
      <div style={{width:"100%",maxWidth:420,animation:"rise .5s",position:"relative",zIndex:1}}>
        ...
        <div className="c" style={{padding:28}}>
          ...
          {rpStep===1 && <React.Fragment>
            ...
            <input type="email" placeholder="Enter your email" value={rpEm} onChange={e=>{setRpEm(e.target.value);setAuthError("")}} autoComplete="email" onKeyDown={e=>{if(e.key==="Enter")doReset()}} style={{marginBottom:16}} />
          ...
          {rpStep===2 && <React.Fragment>
            ...
            <input type="text" placeholder="Enter your answer" value={rpSa} onChange={e=>{setRpSa(e.target.value);setAuthError("")}} autoComplete="off" onKeyDown={e=>{if(e.key==="Enter")doReset()}} style={{marginBottom:16}} />
          ...
          {rpStep===3 && <React.Fragment>
            ...
            <input type="password" placeholder="New password (6+ characters)" value={rpPw} onChange={e=>{setRpPw(e.target.value);setAuthError("")}} autoComplete="new-password" autoCapitalize="none" autoCorrect="off" spellCheck="false" style={{marginBottom:14}} />
            <label style={{fontSize:12,fontWeight:700,color:"#78716c",display:"block",marginBottom:6}}>CONFIRM NEW PASSWORD</label>
            <input type="password" placeholder="Confirm new password" value={rpPc} onChange={e=>{setRpPc(e.target.value);setAuthError("")}} autoComplete="new-password" autoCapitalize="none" autoCorrect="off" spellCheck="false" onKeyDown={e=>{if(e.key==="Enter")doReset()}} style={{marginBottom:16}} />
          ...
          <div style={{textAlign:"center",marginTop:20,fontSize:14,color:"#78716c"}}>
            <span style={{color:"#0e7490",cursor:"pointer",fontWeight:700}} onClick={()=>{setAuthScreen("login");setAuthError("");setRpStep(1);setRpEm("");setRpSa("");setRpPw("");setRpPc("");setRpQ("")}}>← Back to Sign In</span>
          </div>
        </div>
      </div>
    </div>
