      ...
      <div style={{width:'100%',maxWidth:420,animation:'rise .5s',position:'relative',zIndex:1}}>
        ...
        <form className="c" style={{padding:28,position:'relative',overflow:'hidden'}} onSubmit={(e) => { e.preventDefault(); isR ? doReg() : doLog(); }} noValidate>
          ...
          <button
            ...
            style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:10,padding:'13px 20px',marginBottom:16,borderRadius:12,border:'1.5px solid var(--card-b)',background:'var(--card)',cursor:'pointer',fontFamily:"'Outfit',sans-serif",fontSize:'var(--text-md)',fontWeight:600,color:'var(--heading)',boxShadow:'0 1px 3px rgba(0,0,0,.08)',transition:'box-shadow .15s'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
            ...
            <span style={{fontSize:'var(--text-sm)',fontWeight:600,color:'var(--subtext)',whiteSpace:'nowrap'}}>or continue with email</span>
            <div style={{flex:1,height:1,background:'var(--card-b)'}} />
          </div>
          {isR && <React.Fragment>
            <label htmlFor="auth-name" style={{fontSize:'var(--text-sm)',fontWeight:700,color:'var(--subtext)',display:'block',marginBottom:6}}>DISPLAY NAME</label>
          ...
          {isR && pw && (
            <div style={{marginBottom:14}}>
              <div style={{display:'flex',gap:4,marginBottom:4}}>
                {[1,2,3,4].map(i => (
                  <div key={i} style={{flex:1,height:4,borderRadius:4,background:i<=strength?PW_COLORS[strength]:'var(--card-b)',transition:'background .2s'}} />
                ))}
              </div>
              <div style={{fontSize:'var(--text-xs)',fontWeight:700,color:PW_COLORS[strength]}}>{PW_LABELS[strength]}</div>
            </div>
          )}
          ...
          <div style={{textAlign:'center',marginTop:12,fontSize:'var(--text-base)',color:'var(--subtext)'}}>
            {isR?"Have an account? ":"No account? "}
            <button type="button" style={{color:'var(--info)',cursor:'pointer',fontWeight:700,background:'none',border:'none',padding:0,fontSize:'inherit',fontFamily:'inherit'}} onClick={()=>{setAuthScreen(isR?"login":"register");setAuthError("");setAuthEmail("");setPw("");setPc("");setDisplayName("")}}>
