  ...
  return (
    <div>
      ...
      <div style={{
        ...
        borderRadius: 20, padding: '24px 20px', marginBottom: 24, textAlign: 'center',
      }}>
        <div style={{ fontSize: 40, marginBottom: 12 }}>🌏</div>
        <div style={{
          fontSize: 20, fontFamily: "'Playfair Display',serif", fontWeight: 900,
          color: '#fff', marginBottom: 8, lineHeight: 1.3,
        }}>
          If you grew up switching between Croatian and English mid-sentence — that's not a mistake. That's you.
        </div>
      ...
      {/* Heritage truths */}
      <h3 style={{ fontSize: 13, fontWeight: 800, color: 'var(--subtext)', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
        {HERITAGE_TRUTHS.map((t, i) => (
          <div key={i} style={{
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--heading)', marginBottom: 4 }}>{t.title}</div>
      ...
      <h3 style={{ fontSize: 13, fontWeight: 800, color: 'var(--subtext)', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 8 }}>
        Common code-switches — and the "full Croatian"
      </h3>
      <p style={{ fontSize: 12, color: 'var(--subtext)', marginBottom: 14, fontWeight: 500 }}>
        Tap each to see the native Croatian equivalent. Both are valid — knowing the native form gives you more options.
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 28 }}>
        {CODE_SWITCH_EXAMPLES.map((ex, i) => (
          <div key={i} style={{
            background: 'var(--card)', border: `1.5px solid ${activeEx === i ? '#0e7490' : 'var(--card-b)'}`,
            borderRadius: 14, overflow: 'hidden', transition: 'border-color .2s',
          }}>
            <button onClick={() => setActiveEx(activeEx === i ? null : i)} style={{
              width: '100%', padding: '12px 16px', background: 'none', border: 'none',
              cursor: 'pointer', textAlign: 'left', fontFamily: "'Outfit',sans-serif",
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--subtext)', marginBottom: 2, fontStyle: 'italic' }}>
                  "{ex.mixed}"
                </div>
              </div>
              <span style={{ fontSize: 14, color: 'var(--subtext)', opacity: .5 }}>{activeEx === i ? '▲' : '▼'}</span>
            </button>
            {activeEx === i && (
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: 'var(--subtext)', lineHeight: 1.6 }}>
          Whether your Croatian is textbook-perfect or kitchen-table-and-English, it belongs to you. This app just wants to help you use more of it.
        </div>
      </div>
    </div>
  );
