  ...
  return (
    <div className="scr-wrap">
      ...
      {kgTab==="timeline"&&<React.Fragment>
        <div className="c" style={{marginBottom:20,borderLeft:"4px solid #b45309",background:"linear-gradient(135deg,#fffbeb,#fef3c7)"}}>
          <div style={{fontSize:14,lineHeight:1.8,color:"#1c1917"}}>{KINGS.intro}</div>
        </div>
        {KINGS.eras.map(function(e,i){return (
          <div key={i} className="c" style={{marginBottom:12,borderLeft:"4px solid "+(i<2?"#f59e0b":i===2?"#b45309":i===3?"#7c3aed":"#dc2626")}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              <span style={{fontSize:28}}>{e.emoji}</span>
              <div style={{fontSize:17,fontWeight:800,color:"#164e63",fontFamily:"'Playfair Display',serif"}}>{e.title}</div>
            </div>
            <p style={{fontSize:14,lineHeight:1.8,color:"#44403c"}}>{e.text}</p>
          </div>
        );})}
        <h3 className="sh" style={{marginTop:24}}>📌 Key Dates</h3>
        {KINGS.keyFacts.map(function(f,i){return (
          <div key={i} style={{display:"flex",gap:12,padding:"8px 0",borderBottom:"1px solid rgba(0,0,0,.05)"}}>
            <div style={{minWidth:60,fontSize:14,fontWeight:800,color:"#b45309"}}>{f[0]}</div>
            <div style={{fontSize:14,color:"#44403c"}}>{f[1]}</div>
          </div>
        );})}
        <div className="c" style={{marginTop:24,textAlign:"center",borderLeft:"4px solid #b45309",background:"linear-gradient(135deg,#fffbeb,#fef3c7)"}}>
          <div style={{fontSize:20,fontWeight:800,color:"#92400e",fontFamily:"'Playfair Display',serif",fontStyle:"italic",marginBottom:6}}>{KINGS.quote}</div>
          <div style={{fontSize:14,color:"#78716c",fontStyle:"italic"}}>{KINGS.quoteEn}</div>
        </div>
      </React.Fragment>}
      {kgTab==="dukes"&&<React.Fragment>
        <div className="c" style={{marginBottom:16,background:"#fffbeb",borderLeft:"4px solid #f59e0b"}}>
        ...
        {KINGS.dukes.map(function(d,i){return (
          <div key={i} className="c" style={{marginBottom:10,cursor:"pointer"}} onClick={function(){speak(d.name)}}>
      ...
      {kgTab==="kings"&&<React.Fragment>
        <div className="c" style={{marginBottom:16,background:"linear-gradient(135deg,#fffbeb,#fef3c7)",borderLeft:"4px solid #b45309"}}>
        ...
        {KINGS.kings.map(function(k,i){return (
          <div key={i} className="c" style={{marginBottom:12,borderLeft:"4px solid "+(k.color||"#0e7490"),cursor:"pointer"}} onClick={function(){speak(k.name)}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
      ...
      {kgTab==="cities"&&<React.Fragment>
        ...
        {KINGS.royalCities.map(function(c,i){return (
          <div key={i} className="c" style={{marginBottom:10,cursor:"pointer"}} onClick={function(){speak(c.name)}}>
