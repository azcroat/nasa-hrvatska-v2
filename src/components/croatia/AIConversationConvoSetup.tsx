...
export default function AIConversationConvoSetup({
  ...
  return (
    <div className="scr-wrap">
      ...
      >
        {filteredScenarios.length} scenario{filteredScenarios.length !== 1 ? 's' : ''} for level{' '}
        {level}
                ...
                >
                  <div
                    style={{
                      ...
                      inset: 0,
                      background:
                        'linear-gradient(90deg,rgba(0,0,0,.55) 0%,rgba(0,0,0,.1) 55%,rgba(0,0,0,.25) 100%)',
                    }}
                  />
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        alignItems: 'center',
                        justifyContent: 'center',
                        background: `linear-gradient(135deg,${s.color}88,${s.color}44)`,
                        fontSize: 28,
                      }}
                    >
                      {s.icon}
      ...
      >
        ...
        {showCustom && (
          ...
          >
            <div
              style={{ fontSize: 12, fontWeight: 700, color: 'var(--subtext)', marginBottom: 8 }}
            >
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
        {[
          ...
          >
            {label}
            {sub && <span style={{ fontSize: 'var(--text-xs)', opacity: 0.7 }}>{sub}</span>}
          </span>
        ))}
      </div>
      ...
      {!isOnline && (
        ...
        >
          <span>📶</span>
          <span>You're offline — AI conversation requires an internet connection.</span>
        </div>
      )}
      <button
        className="b bp"
        ...
        onClick={onStart}
        disabled={!scenario || !isOnline}
      >
        {!isOnline
          ? 'Connect to the internet to start'
          : scenario
            ? `Start — ${scenario.title} (${level})`
