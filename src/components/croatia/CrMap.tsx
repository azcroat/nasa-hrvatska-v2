...
export default function CrMap({ goBack }) {
  ...
  return (
    <div className="scr-wrap">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
        <button
          style={{
            padding: '6px 14px',
            borderRadius: 10,
            border: '2px solid ' + (mapCat === 'all' ? '#164e63' : '#e7e5e4'),
            background: mapCat === 'all' ? '#164e63' : 'white',
            color: mapCat === 'all' ? 'white' : '#44403c',
            fontWeight: 600,
            fontSize: 12,
            cursor: 'pointer',
          }}
        ...
        {MAPPLACES.categories.map(function (cat) {
          return (
            <button
              key={cat.id}
              style={{
      ...
      {MAPPLACES.places
        .filter(function (p) {
          return mapCat === 'all' || p.cat === mapCat;
        })
        .map(function (p, i) {
          const catInfo = MAPPLACES.categories.find(function (c) {
            return c.id === p.cat;
          });
          return (
            <button
              ...
              style={{
                marginBottom: 8,
                padding: '12px 14px',
                borderLeft: '4px solid ' + (catInfo ? catInfo.color : '#e7e5e4'),
              }}
              onClick={function () {
                setMapSel(mapSel === i ? null : i);
              }}
            >
              <div
