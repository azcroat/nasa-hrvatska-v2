...
function getRecentMedia() {
  try {
    return JSON.parse(localStorage.getItem('nh_media_used') || '{}');
  } catch {
    return {};
  }
}
...
const GENRE_VOCAB = {
  tv: [
    { hr: 'vijesti', en: 'news' },
    { hr: 'voditeljica', en: 'presenter (f)' },
    { hr: 'emisija', en: 'show / programme' },
    { hr: 'prilog', en: 'report / segment' },
    { hr: 'prognoza', en: 'forecast' },
    { hr: 'gledati', en: 'to watch' },
  ],
  music: [
    { hr: 'pjesma', en: 'song' },
    { hr: 'pjevač', en: 'singer (m)' },
    { hr: 'glazba', en: 'music' },
    { hr: 'stih', en: 'lyric / verse' },
    { hr: 'klapa', en: 'Dalmatian a cappella group' },
    { hr: 'ritam', en: 'rhythm' },
  ],
  film: [
    { hr: 'film', en: 'film / movie' },
    { hr: 'glumac', en: 'actor (m)' },
    { hr: 'redatelj', en: 'director (m)' },
    { hr: 'scena', en: 'scene' },
    { hr: 'titl', en: 'subtitle' },
    { hr: 'nagrade', en: 'awards' },
  ],
  sport: [
    { hr: 'utakmica', en: 'match / game' },
    { hr: 'gol', en: 'goal' },
    { hr: 'igrač', en: 'player (m)' },
    { hr: 'pobjednik', en: 'winner (m)' },
    { hr: 'liga', en: 'league' },
    { hr: 'navijač', en: 'fan / supporter' },
  ],
  podcast: [
    { hr: 'epizoda', en: 'episode' },
    { hr: 'domaćin', en: 'host (m)' },
    { hr: 'gost', en: 'guest' },
    { hr: 'razgovor', en: 'conversation' },
    { hr: 'tema', en: 'topic / theme' },
    { hr: 'slušati', en: 'to listen' },
  ],
  culture: [
    { hr: 'kultura', en: 'culture' },
    { hr: 'tradicija', en: 'tradition' },
    { hr: 'baština', en: 'heritage' },
    { hr: 'festival', en: 'festival' },
    { hr: 'povijest', en: 'history' },
    { hr: 'umjetnost', en: 'art' },
  ],
};
...
function GridCard({ m, onOpen }: { m: MediaItem; onOpen: () => void }) {
  return (
    <button
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.transform = 'translateY(-2px)';
        e.currentTarget.style.boxShadow = `0 8px 28px ${m.color}28`;
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.transform = '';
    ...
    <React.Fragment>
      ...
      >
        <div
          style={{
            height: 4,
            background: 'linear-gradient(90deg,#D40030 0%,#D40030 50%,#F8F6F2 50%,#F8F6F2 100%)',
          }}
        />
        <div
          style={{
            ...
            inset: 0,
            background:
              'linear-gradient(105deg,transparent 30%,rgba(255,255,255,.035) 50%,transparent 70%)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <div
              style={{
                ...
                fontSize: 18,
                flexShrink: 0,
                boxShadow: '0 4px 12px rgba(30,215,96,.4)',
              }}
            >
              🎵
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 900, color: 'var(--heading)' }}>
                Croatian Music on Spotify
              </div>
              <div style={{ fontSize: 11, color: 'var(--subtext)', marginTop: 1 }}>
                14 curated playlists · tap to expand
              </div>
            </div>
          </div>
          <SpotifySection />
        ...
        return (
          <div key={cat} style={{ marginBottom: 24, animation: 'nh-fade-in .35s ease both' }}>
            ...
            >
              ...
              <span style={{ fontSize: 14, fontWeight: 900, color: 'var(--heading)', flex: 1 }}>
                {meta.title}
              </span>
              {userGoal && (
                <span style={{ fontSize: 9, color: 'var(--subtext)', fontStyle: 'italic' }}>
                  sorted for you
                </span>
              )}
              <span
