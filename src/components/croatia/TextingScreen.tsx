      ...
      <div className="scr-wrap" style={{ textAlign: 'center', paddingTop: 40 }}>
        ...
        <p style={{ color: 'var(--subtext)', fontSize: 14 }}>
          {score === questions.length
            ? 'Savršeno! You text like a local! 🇭🇷'
            : "Keep practising — locals will think you're one of them soon!"}
        </p>
        <button className="b bp" style={{ marginTop: 24, marginRight: 12 }} onClick={startQuiz}>
          Try Again
        </button>
        <button
          className="b"
      ...
      <div className="scr-wrap">
        ...
        <div className="c" style={{ marginTop: 8 }}>
          ...
          {q.o.map((o, i) => (
            <button
              key={i}
              className={'ob ' + (answered ? (i === q.c ? 'ok' : selected === i ? 'no' : '') : '')}
              onClick={() => pick(i)}
            >
              {o}
            </button>
