    ...
    <svg viewBox={`0 0 100 ${height}`} preserveAspectRatio="none" style={{ width: '100%', height }}>
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect
              x={i * w + w * 0.1} y={height - barH - 2}
              width={w * 0.8} height={Math.max(barH, 1)}
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
        {[
          ...
          { label: 'vs Last Week', value: trend >= 0 ? `▲ ${trend}%` : `▼ ${Math.abs(trend)}%`, color: trend >= 0 ? '#16a34a' : '#dc2626' },
        ].map(({ label, value, color }) => (
          <div key={label} className="c" style={{ textAlign: 'center', padding: '12px 8px' }}>
            <div style={{ fontSize: 18, fontWeight: 900, color: color || 'var(--heading)' }}>{value}</div>
            <div style={{ fontSize: 10, color: 'var(--subtext)', fontWeight: 600, marginTop: 3 }}>{label}</div>
          </div>
        ))}
      </div>

      <div className="c" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--subtext)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.06em' }}>
          XP — Last 30 Days
        </div>
        <SVGBarChart data={history.map(d => ({ value: d.delta || 0, today: d.today }))} />
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, fontSize: 10, color: 'var(--subtext)' }}>
