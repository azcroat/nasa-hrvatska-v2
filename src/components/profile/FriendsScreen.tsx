    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          {isMe && (
            <span style={{ color: 'var(--info)', fontSize: 'var(--text-xs)', flexShrink: 0 }}>
              (You)
            </span>
          )}
        </div>
...
function FamilyTab({ user }) {
  ...
  async function handleJoin() {
    ...
    setError('');
    const _wkd = new Date();
    const _wkday = _wkd.getDay() || 7;
    _wkd.setDate(_wkd.getDate() + 4 - _wkday);
    const _wkyear = _wkd.getFullYear();
    const _wknum = Math.ceil(
      ((_wkd.getTime() - new Date(_wkyear, 0, 1).getTime()) / 86400000 + 1) / 7,
    );
    ...
    <div>
      <div
        style={{
          ...
          border: '1px solid var(--info-b, rgba(14,116,144,.3))',
          borderRadius: 16,
          padding: 20,
          marginBottom: 20,
        }}
      >
        ...
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--subtext)', marginTop: 8 }}>
          Share this code with family or friends so they can join
        </div>
      </div>
      ...
      <button
        ...
        style={{
          ...
          padding: 12,
          borderRadius: 12,
          border: '1.5px solid var(--error-b, #dc2626)',
          background: 'transparent',
          color: 'var(--error)',
          fontWeight: 700,
          cursor: 'pointer',
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--subtext)', marginTop: 8 }}>
          Share this with anyone to connect — they'll enter it below
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {error && (
          <div style={{ color: 'var(--error)', fontSize: 'var(--text-xs)', marginTop: 8 }}>
            {error}
          </div>
        )}
      </div>
      ...
      {loadingFriends ? (
        ...
        >
          ...
          <div style={{ color: 'var(--subtext)', fontSize: 'var(--text-sm)', lineHeight: 1.6 }}>
            ...
            <br />
            or enter a friend's code to connect.
          </div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
...
export default function FriendsScreen({ user, goBack }) {
  ...
  return (
    <div className="scr-wrap">
      ...
      {activeTab === 'friends' ? <FriendsTab user={user} /> : <FamilyTab user={user} />}
    </div>
  );
}
