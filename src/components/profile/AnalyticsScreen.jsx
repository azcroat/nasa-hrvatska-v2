...
import { H, getMistakes, getDueReviews, BADGES } from '../../data.jsx';
...
function StatTile({ icon, value, label, color }) {
  return (
    <div style={{ flex: 1, background: '#fff', borderRadius: 16, padding: '16px 12px', textAlign: 'center', border: '1.5px solid #e2e8f0', minWidth: 0 }}>
...
function BadgeRow({ badge, earned }) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: earned ? '#78350f' : '#9ca3af' }}>{badge.n}</div>
        <div style={{ fontSize: 11, color: '#9ca3af' }}>{badge.d}</div>
      </div>
      {earned && <div style={{ fontSize: 11, fontWeight: 700, color: '#d97706', background: '#fef3c7', borderRadius: 20, padding: '2px 8px' }}>✓ Earned</div>}
    </div>
  );
}
  ...
  return (
    <div className="scr-wrap">
      ...
      <div className="c" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#1e293b', marginBottom: 14 }}>🧠 Vocabulary Mastery</div>
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
          <div style={{ flex: 1, textAlign: 'center', background: '#f0fdf4', borderRadius: 12, padding: '12px 8px', border: '1px solid #bbf7d0' }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', background: '#eff6ff', borderRadius: 12, padding: '12px 8px', border: '1px solid #bfdbfe' }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', background: '#fff7ed', borderRadius: 12, padding: '12px 8px', border: '1px solid #fed7aa' }}>
        ...
        {dueWords.length > 0 && (
          <div style={{ background: '#eff6ff', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#1d4ed8', fontWeight: 600 }}>
      ...
      {/* Streak stats */}
      <div className="c" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#1e293b', marginBottom: 14 }}>🔥 Streaks</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ flex: 1, textAlign: 'center', background: 'linear-gradient(135deg,#fff7ed,#fed7aa)', borderRadius: 12, padding: '14px 8px', border: '1px solid #fdba74' }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', background: 'linear-gradient(135deg,#fefce8,#fef9c3)', borderRadius: 12, padding: '14px 8px', border: '1px solid #fde047' }}>
            <div style={{ fontSize: 26, fontWeight: 800, color: '#b45309' }}>{longestStreak} 🏆</div>
      ...
      {/* Category breakdown */}
      <div className="c" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#1e293b', marginBottom: 14 }}>📈 Lessons by Category</div>
        {categoryBreakdown.map(cat => (
          <MiniBar key={cat.label} label={cat.label} value={cat.value} max={maxCat} color={cat.color} icon={cat.icon} />
        ))}
      </div>
      ...
      <div className="c" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: '#d97706' }}>{earnedBadges.size}/{BADGES.length} earned</div>
        </div>
        {/* Earned first */}
        {BADGES.filter(b => earnedBadges.has(b.id)).map(b => <BadgeRow key={b.id} badge={b} earned />)}
        {BADGES.filter(b => !earnedBadges.has(b.id)).map(b => <BadgeRow key={b.id} badge={b} earned={false} />)}
      </div>

      {/* Top mistakes */}
      {mistakes.length > 0 && (
        <div className="c" style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#1e293b', marginBottom: 12 }}>⚠️ Most Missed Words</div>
          {mistakes.slice(0, 5).map(m => (
            <div key={m.hr} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
              <div>
                <span style={{ fontSize: 15, fontWeight: 700, color: '#1e40af' }}>{m.hr}</span>
                {m.en && <span style={{ fontSize: 13, color: '#6b7280', marginLeft: 8 }}>— {m.en}</span>}
              </div>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#dc2626', background: '#fee2e2', borderRadius: 20, padding: '2px 8px' }}>×{m.count}</div>
            </div>
          ))}
        </div>
      )}
