...
const ALL_TIERS = [
  { id: 'diamond', name: 'Diamond', icon: '💎', xp: '600+', desc: 'Top 3 promoted to next tier' },
  {
    ...
    icon: '🏆',
    xp: '300–599',
    desc: 'Top 3 promoted to Diamond',
  },
...
export default function WeeklyLeague({ authUser: au, name, stats, goBack }) {
  ...
  const handleJoin = useCallback(async () => {
    if (!au) {
      setError('You must be logged in to join the league.');
      return;
    }
    setJoining(true);
    setError('');
    ...
    return (
      <div className="scr-wrap">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 20, fontWeight: 900, color: 'var(--heading)' }}>
              Weekly League
            </div>
            <div style={{ fontSize: 12, color: 'var(--subtext)', fontWeight: 600 }}>
              Resets Monday · {formatCountdown(countdown)} left
            </div>
          </div>
          <div
            style={{
        ...
        >
          ...
          <div style={{ fontSize: 13, opacity: 0.85, marginBottom: 20, lineHeight: 1.5 }}>
            Compete against 29 other learners in your tier.
            <br />
            Top 3 get promoted. Bottom 5 drop a tier.
          </div>
          <div style={{ fontSize: 22, fontWeight: 900, marginBottom: 4 }}>{myWeekXP} XP</div>
          <div style={{ fontSize: 11, opacity: 0.7, marginBottom: 20 }}>earned this week</div>
          <button
            onClick={handleJoin}
            disabled={joining || !au}
            style={{
              ...
              fontSize: 15,
              fontWeight: 900,
              cursor: joining || !au ? 'not-allowed' : 'pointer',
              opacity: joining || !au ? 0.6 : 1,
  ...
  const _safeFromDemotion =
    (data.demotionXP || 0) > myWeekXP ? Math.max(0, (data.demotionXP || 0) - myWeekXP) : null;
    ...
    <div className="scr-wrap">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--subtext)', fontWeight: 600 }}>
            Resets Monday ·{' '}
            <span style={{ color: '#e53e3e', fontWeight: 800 }}>{formatCountdown(countdown)}</span>{' '}
            left
          </div>
        </div>
        <button
      ...
      >
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 10, opacity: 0.75, fontWeight: 700, textTransform: 'uppercase' }}>
            XP this week
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        ...
        <div
          style={{
            flex: 1,
            background: data.inPromotionZone ? 'rgba(22,163,74,.1)' : 'var(--card)',
        ...
        >
          <div
            style={{
              fontSize: 11,
              fontWeight: 800,
              color: data.inPromotionZone ? '#16a34a' : 'var(--subtext)',
              marginBottom: 3,
            }}
          >
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--heading)' }}>
            {data.inPromotionZone ? 'Top 3 — keep it up!' : `Need ${xpToPromo} more XP`}
          </div>
          <div style={{ fontSize: 10, color: 'var(--subtext)', marginTop: 2 }}>
            Top 3 advance next week
          </div>
        </div>
        {/* Demotion */}
        <div
          style={{
            flex: 1,
            background: data.inDemotionZone ? 'rgba(220,38,38,.06)' : 'var(--card)',
        ...
        >
          <div
            style={{
              fontSize: 11,
              fontWeight: 800,
              color: data.inDemotionZone ? '#dc2626' : 'var(--subtext)',
              marginBottom: 3,
            }}
          >
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--heading)' }}>
            {data.inDemotionZone ? 'Earn more to stay!' : 'Not in bottom 5'}
          </div>
          <div style={{ fontSize: 10, color: 'var(--subtext)', marginTop: 2 }}>
            Bottom 5 drop a tier
          </div>
        </div>
      </div>
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: `${Math.round((m.xp / maxXP) * 100)}%`,
                          background: inDemotion ? '#e53e3e' : '#0e7490',
                          borderRadius: 2,
                        }}
                      />
                    </div>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div
                      style={{
                        fontSize: 15,
                        fontWeight: 800,
                        color: inDemotion ? '#e53e3e' : '#0e7490',
                      }}
                    >
                      {m.xp}
                    </div>
      ...
      {showTiers && (
        ...
        >
          <div style={{ fontSize: 12, color: 'var(--subtext)', marginBottom: 12, lineHeight: 1.6 }}>
            Each week you compete in a group of up to 30 learners. Earn XP by completing lessons,
            streaks, and quests.
            <br />
            <br />
            <strong style={{ color: '#16a34a' }}>Top 3</strong> get promoted to the next tier next
            week.
            <br />
                  ...
                  <div style={{ flex: 1 }}>
                    <div
                      style={{
                        fontSize: 12,
                        fontWeight: 800,
                        color: isCurrent ? tm.color : 'var(--heading)',
                      }}
                    >
                      {t.name}
                      {isCurrent && (
