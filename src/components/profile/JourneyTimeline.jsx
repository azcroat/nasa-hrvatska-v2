...
import { getJourneyMilestones } from '../../data.jsx';

const MILESTONE_ICONS = {
  first_lesson: { icon: '📚', label: 'First Lesson', msg: 'Your Croatian journey begins!' },
  first_speaking: { icon: '🎤', label: 'First Speaking', msg: 'You spoke Croatian for the first time!' },
...
export default function JourneyTimeline() {
  ...
  if (milestones.length === 0) {
    return (
      <div style={{ textAlign:'center', padding:'24px 16px', color:'var(--subtext)' }}>
        ...
        <div style={{ fontSize:13, fontWeight:600 }}>Your journey starts with your first lesson.</div>
        <div style={{ fontSize:11, marginTop:4 }}>Milestones will appear here as you progress.</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ position:'relative', paddingLeft:24 }}>
      {/* Vertical line */}
      <div style={{ position:'absolute', left:8, top:0, bottom:0, width:2, background:'var(--bar-bg)', borderRadius:1 }} />
          ...
          <div key={i} style={{ position:'relative', marginBottom:16, paddingLeft:20 }}>
            ...
            <div style={{
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:2 }}>
                ...
                <div>
                  <div style={{ fontSize:13, fontWeight:800, color:'var(--heading)' }}>{def.label}</div>
                  <div style={{ fontSize:10, color:'var(--subtext)', fontWeight:600 }}>{dateStr}</div>
                </div>
              </div>
              <div style={{ fontSize:11, color:'var(--subtext)', fontStyle:'italic' }}>{def.msg}</div>
            </div>
          </div>
        );
      })}
      ...
      <div style={{ position:'relative', paddingLeft:20 }}>
        <div style={{ position:'absolute', left:-4, top:4, width:16, height:16, borderRadius:'50%', background:'var(--bar-bg)', border:'2px solid var(--card)' }} />
        <div style={{ fontSize:11, color:'var(--subtext)', fontStyle:'italic', padding:'8px 0' }}>
