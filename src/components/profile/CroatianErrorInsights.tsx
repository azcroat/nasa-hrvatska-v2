...
function ErrorCard({
  ...
  const urgencyColor =
    urgency === 'high' ? '#D4002D' : urgency === 'medium' ? '#d97706' : '#16a34a';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 14, color: 'var(--subtext)', opacity: 0.7, flexShrink: 0 }}>
          {expanded ? '▲' : '▼'}
        </span>
      </button>
...
function EmptyInsights() {
  return (
    ...
    >
      ...
      >
        Complete a few typing and quiz exercises. The app will automatically track your
        Croatian-specific error patterns and show them here.
      </div>
    </div>
  );
}
              ...
              {tab.count > 0 && (
                <span
                  style={{
                    fontSize: 10,
                    fontWeight: 900,
                    background: activeTab === tab.key ? 'rgba(255,255,255,.25)' : 'var(--card-b)',
                    borderRadius: 10,
                    padding: '1px 6px',
                    color: activeTab === tab.key ? '#fff' : 'var(--subtext)',
                  }}
        ...
        <>
          {weakTopics.length === 0 ? (
            ...
            <>
              ...
              >
                Topics where your accuracy is below 65% — sorted worst first. Drill these to unlock
                faster progress.
              </div>
              {weakTopics.slice(0, 8).map((topic) => (
                <WeakTopicCard key={topic.id} topic={topic} onPractice={handlePractice} />
              ))}
            </>
          )}
        </>
