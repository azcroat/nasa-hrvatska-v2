// media.js
// ytId  — YouTube video ID for inline iframe embed (highest priority)
// ytList — YouTube playlist ID for inline embed
// stream — relative URL for live radio (RadioPlayer)
// web    — external URL, opened via @capacitor/browser
// scr    — internal screen name (setScr navigation)
export const MEDIA = [
  // ─── TV & NEWS ───────────────────────────────────────────────────────────────
  {
    name: 'HRT 1 — Live TV',
    desc: 'Main national channel • sign in to HRTI to watch • some live content has broadcast restrictions',
    web: 'https://hrti.hrt.hr/home',
    icon: '📺',
    color: '#0e7490',
    cat: 'tv',
    level: 'B1',
    video: true,
    tip: 'Watch the Dnevnik (news at 19:30) daily. Same stories as what you read online — hearing the words cements vocabulary fast.',
  },
  {
    name: 'HRT 2 — Kultura & Sport',
    desc: 'Arts, documentaries, sports coverage • sign in to HRTI to watch',
    web: 'https://hrti.hrt.hr/home',
    icon: '📺',
    color: '#0284c7',
    cat: 'tv',
    level: 'B2',
    video: true,
    tip: 'HRT 2 airs cultural programs and in-depth debates. Great for hearing formal, careful Croatian.',
  },
  {
    name: 'HRT 3 — Documentaries',
    desc: 'Education, nature & travel docs • sign in to HRTI to watch',
    web: 'https://hrti.hrt.hr/home',
    icon: '🎓',
    color: '#0369a1',
    cat: 'tv',
    level: 'B1',
    video: true,
    tip: 'Documentary narration is slow, clear and well-pronounced. Perfect for intermediate learners.',
  },
  {
    name: 'HRT Vijesti — News 24/7',
    desc: 'Rolling news — headlines every hour',
    web: 'https://vijesti.hrt.hr/',
    icon: '📰',
    color: '#164e63',
    cat: 'tv',
    level: 'B1',
    tip: 'Bookmark this and read one article every morning. News Croatian is formal and consistent — ideal for building B1→B2 vocabulary.',
  },
  {
    name: 'Dnevnik.hr',
    desc: 'Daily Croatian news portal',
    web: 'https://dnevnik.hr/',
    icon: '📰',
    color: '#b91c1c',
    cat: 'tv',
    level: 'A2',
    tip: 'Short, punchy headlines use everyday vocabulary. Great for A2 learners — scan headlines first, then read one full article.',
  },
  {
    name: '24sata',
    desc: 'Most-read Croatian portal • casual language',
    web: 'https://www.24sata.hr/',
    icon: '📰',
    color: '#dc2626',
    cat: 'tv',
    level: 'A2',
    tip: '24sata uses colloquial, everyday Croatian — very close to how people actually speak. Start here if you feel newspapers are too hard.',
  },
  {
    name: 'Jutarnji list',
    desc: 'Quality morning newspaper',
    web: 'https://www.jutarnji.hr/',
    icon: '📰',
    color: '#1e40af',
    cat: 'tv',
    level: 'C1',
    tip: 'Jutarnji opinion pieces use sophisticated sentence structures. Aim for this at C1 level when you want to read native-level journalism.',
  },
  {
    name: 'Večernji list',
    desc: 'Evening newspaper — balanced coverage',
    web: 'https://www.vecernji.hr/',
    icon: '📰',
    color: '#7c3aed',
    cat: 'tv',
    level: 'B2',
    tip: 'Večernji has good local and regional stories. Reading it alongside HRT news gives you broader exposure to written Croatian styles.',
  },
  {
    name: 'Index.hr',
    desc: 'Independent news & opinion portal',
    web: 'https://www.index.hr/',
    icon: '📰',
    color: '#ea580c',
    cat: 'tv',
    level: 'B2',
    tip: "Index.hr features debate and commentary — you'll encounter argumentation structures and rhetorical vocabulary here.",
  },
  {
    name: 'Glas Hrvatske',
    desc: 'Voice of Croatia • English-Croatian bilingual',
    web: 'https://glashrvatske.hrt.hr/en/',
    icon: '🌍',
    color: '#1d4ed8',
    cat: 'tv',
    level: 'A1',
    tip: 'Perfect for absolute beginners. Read the English version of a story, then the Croatian one side-by-side. Builds confidence fast.',
  },
  // ─── MUSIC & RADIO ───────────────────────────────────────────────────────────
  {
    name: 'HRT Radio 1',
    desc: 'Croatian public radio — news, talk, music',
    web: 'https://radio.hrt.hr',
    stream: '/radio?s=hrt1',
    icon: '📻',
    color: '#0e7490',
    cat: 'music',
    level: 'B1',
    live: true,
    tip: 'Leave HRT Radio 1 playing in the background while you cook or drive. Passive listening is powerful — your brain absorbs rhythm and pronunciation.',
  },
  {
    name: 'HRT Radio 2 — Classic',
    desc: 'Classical music & cultural programs',
    web: 'https://radio.hrt.hr',
    stream: '/radio?s=hrt2',
    icon: '🎵',
    color: '#6d28d9',
    cat: 'music',
    level: 'B2',
    live: true,
    tip: 'Cultural programs feature thoughtful, slow speech from educated speakers. Great for training your ear on formal Croatian.',
  },
  {
    name: 'CMC TV',
    desc: 'Croatian Music Channel • pop, folk, turbofolk',
    web: 'https://cmc.com.hr/program/cmc-tv-stream/',
    icon: '🎶',
    color: '#7c3aed',
    cat: 'music',
    level: 'A1',
    live: true,
    tip: 'Music is the best passive learning tool. Even at A1, exposure to Croatian song structures trains your ear for rhythm, stress and melody.',
  },
  {
    name: 'CMC Radio Live',
    desc: '24/7 Croatian hits • all genres',
    web: 'https://radio.cmc.com.hr',
    stream: '/radio?s=cmc',
    icon: '🎧',
    color: '#9333ea',
    cat: 'music',
    level: 'A1',
    live: true,
    tip: 'Pop songs repeat the same phrases dozens of times — this is vocabulary acquisition without effort. Pick one song and look up the lyrics.',
  },
  // ytId: "Moja Zemlja" official live — one of Thompson's most iconic performances
  {
    name: 'Thompson',
    desc: 'Patriotic Croatian rock • Marko Perković Thompson',
    web: 'https://www.youtube.com/results?search_query=thompson+moja+zemlja+croatian',
    ytId: 'w-bFLUdT0P0',
    icon: '🇭🇷',
    color: '#b91c1c',
    cat: 'music',
    level: 'A2',
    tip: "Thompson's lyrics tell Croatian stories with deep cultural meaning. Look up 'Geni Jadranskog mora' or 'Lijepa li si' for beautiful vocabulary.",
  },
  // ytId: "Cesarica" — Oliver's most-loved song, official upload
  {
    name: 'Oliver Dragojević',
    desc: 'Legendary Dalmatian singer • timeless classics',
    web: 'https://www.youtube.com/results?search_query=Oliver+Dragojevic+cesarica+official',
    ytId: '9P_K0MBHuW8',
    icon: '🎤',
    color: '#0369a1',
    cat: 'music',
    level: 'A2',
    tip: "Oliver sang in both standard Croatian and Dalmatian dialect. His music is Croatia's soul — the lyrics are poetry-level vocabulary.",
  },
  // ytId: "Balada o dobroj ženi" — Gibonni classic
  {
    name: 'Gibonni',
    desc: 'Modern Dalmatian pop & rock',
    web: 'https://www.youtube.com/results?search_query=Gibonni+best+songs+Croatian+lyrics',
    ytId: '1Q5FiMMPBGw',
    icon: '🎸',
    color: '#059669',
    cat: 'music',
    level: 'B1',
    tip: "Gibonni's lyrics use beautiful metaphors and intermediate vocabulary. Find a song you love and dissect every line.",
  },
  {
    name: 'Hrvatska Glazba Mix',
    desc: 'Wide Croatian music mix on YouTube',
    web: 'https://www.youtube.com/results?search_query=hrvatska+glazba+mix+2024+narodna',
    icon: '🎵',
    color: '#db2777',
    cat: 'music',
    level: 'A1',
    tip: 'Expose yourself to a broad range of styles — folk, pop, dalmatian, slavonian. Each region has its own musical vocabulary.',
  },
  // ─── FILM & SERIES ───────────────────────────────────────────────────────────
  {
    name: 'Croatian Films (Free)',
    desc: '250+ award-winning shorts • English subtitles available',
    web: 'https://www.croatian.film/en/films',
    icon: '🎬',
    color: '#b91c1c',
    cat: 'film',
    level: 'B1',
    tip: 'Start with short films (5-15 min). Watch once with English subs for story, then again with Croatian subs for language. Brilliant free resource.',
  },
  {
    name: 'Hrvatski Filmovi',
    desc: 'Full Croatian feature films on YouTube',
    web: 'https://www.youtube.com/results?search_query=hrvatski+film+s+prijevodom+cijeli+film',
    icon: '🎥',
    color: '#dc2626',
    cat: 'film',
    level: 'B2',
    tip: 'Feature films expose you to natural conversation, regional dialects and slang. At B2, try watching the first 10 min without subtitles.',
  },
  // ytId: "Što je Hrvatska?" HRT travel documentary episode — Croatia 365 series
  {
    name: 'Što je Hrvatska?',
    desc: 'Discover Croatia — travel & culture series',
    web: 'https://www.youtube.com/results?search_query=sto+je+hrvatska+dokumentarac+hrt',
    ytId: 'aFsVGT-LHQY',
    icon: '🏔️',
    color: '#0e7490',
    cat: 'film',
    level: 'A2',
    tip: 'Travel documentaries use visual context to help you understand without every word. Ideal bridge from A2 to B1 listening comprehension.',
  },
  {
    name: 'Kvart Priča',
    desc: 'Zagreb neighborhoods — documentary series',
    web: 'https://www.youtube.com/results?search_query=kvart+prica+zagreb+dokumentarac',
    icon: '🏙️',
    color: '#7c3aed',
    cat: 'film',
    level: 'B1',
    tip: 'Real people talking about their neighborhoods in natural, unscripted Croatian. This is authentic speech — invaluable at B1+.',
  },
  // ytId: Easy Croatian episode 1 — street interviews with Croatian subtitles (official Easy Croatian channel)
  {
    name: 'Easy Croatian',
    desc: 'Street interviews with subtitles • YouTube',
    web: 'https://www.youtube.com/c/EasyCroatian',
    ytId: 'RjD2JHmSIp8',
    icon: '🎙️',
    color: '#16a34a',
    cat: 'film',
    level: 'A2',
    tip: 'Short street interviews (5-10 min) with full Croatian and English subtitles. You hear real people, not actors. Start here for authentic speech.',
  },
  // ytId: Basic Croatian phrases lesson (Learn Croatian with CroatianABC)
  {
    name: 'Croatian Learning — YouTube',
    desc: 'Croatian language tutorials & lessons',
    web: 'https://www.youtube.com/results?search_query=learn+croatian+language+lessons+youtube',
    ytId: 'c1nJBbBS9aA',
    icon: '📚',
    color: '#0284c7',
    cat: 'film',
    level: 'A1',
    tip: "Structured video lessons reinforce what you learn in the app. Use YouTube to see grammar explained visually when a concept isn't clicking.",
  },
  // ─── SPORT ───────────────────────────────────────────────────────────────────
  {
    name: 'HNL & Vatreni',
    desc: 'Football: standings, clubs, national team vocab',
    web: '',
    icon: '⚽',
    color: '#003da5',
    cat: 'sport',
    level: 'A2',
    scr: 'football',
    tip: "Sport commentary is fast but repetitive — you'll quickly absorb 'zabiti gol', 'žuti karton', 'napad', 'obrana'. Croatian sports vocabulary is great fun.",
  },
  {
    name: 'HNK Dinamo Zagreb',
    desc: 'Most successful Croatian club',
    web: 'https://www.nk-dinamo.hr/',
    icon: '💙',
    color: '#003da5',
    cat: 'sport',
    level: 'B1',
    tip: "Club websites are written in natural sports Croatian. Follow a club's news section to absorb vocabulary in context you care about.",
  },
  {
    name: 'HNK Hajduk Split',
    desc: "The people's club of Dalmatia",
    web: 'https://hajduk.hr/',
    icon: '⚽',
    color: '#dc2626',
    cat: 'sport',
    level: 'B1',
    tip: "Hajduk's passionate fan culture uses rich Dalmatian idioms online. Their social media is a window into how Dalmatians actually speak and write.",
  },
  {
    name: 'Croatian Basketball',
    desc: 'HKS, ABA Liga, Premijer Liga',
    web: 'https://www.hks-cbf.hr/',
    icon: '🏀',
    color: '#f97316',
    cat: 'sport',
    level: 'B1',
    tip: 'Basketball terms are often international (koš, asistencija, trojka) making them easy entry points. Follow a player you know — Jokić plays against Croatian players regularly.',
  },
  // ytId: Dražen Petrović documentary — "The Mozart of Basketball"
  {
    name: 'Basketball Legends',
    desc: 'Dražen, Kukoč, Rađa, Bogdanović',
    web: 'https://www.youtube.com/results?search_query=croatian+basketball+legends+dražen+petrovic+kukoc+highlights',
    ytId: 'cQALdp0iCv8',
    icon: '🏆',
    color: '#ea580c',
    cat: 'sport',
    level: 'A2',
    tip: 'Watch highlights with Croatian commentary on YouTube. The emotional language around legends like Dražen Petrović teaches you exactly how Croatians express pride and passion.',
  },
  {
    name: 'ABA Liga Highlights',
    desc: 'Official match highlights & video clips',
    web: 'https://www.aba-liga.com/newslist/videos/1/',
    icon: '🏀',
    color: '#0e7490',
    cat: 'sport',
    level: 'B1',
    video: true,
    tip: 'Watch the highlights with Croatian commentary — fast, repetitive basketball language is perfect for absorbing sports vocabulary in context.',
  },
  {
    name: 'Croatians in U.S. College Basketball',
    desc: 'Croatian-born players on NCAA & NAIA rosters right now — D1 through NAIA, with bios & stats',
    scr: 'croatiaathletes',
    icon: '🇭🇷',
    color: '#003da5',
    cat: 'sport',
    level: 'A1',
    tip: 'Follow these players to stay connected to Croatian basketball culture while learning — their school websites and ESPN pages are full of Croatian-language content opportunities.',
  },
  // ─── PODCASTS & LEARNING ─────────────────────────────────────────────────────
  // ytId: Slow Croatian for Learners — beginner-friendly episode
  {
    name: 'Slow Croatian Podcast',
    desc: 'Slow, clear Croatian speech for learners',
    web: 'https://www.youtube.com/results?search_query=slow+croatian+podcast+for+learners',
    ytId: '7CWBT8s6t9I',
    icon: '🎙️',
    color: '#16a34a',
    cat: 'podcast',
    level: 'A2',
    tip: 'Podcasts recorded slowly for learners are gold. Even 10 minutes a day builds listening stamina dramatically over weeks.',
  },
  {
    name: 'HRT Podcast Arhiva',
    desc: 'Croatian radio podcasts on demand',
    web: 'https://radio.hrt.hr',
    icon: '🎧',
    color: '#0e7490',
    cat: 'podcast',
    level: 'B2',
    tip: 'HRT podcasts cover everything from history to cooking. Find a topic you love and listen to it — motivation sustains learning better than discipline.',
  },
  // ytId: Croatian bedtime stories — popular channel for learners
  {
    name: 'Priče Za Laku Noć',
    desc: 'Croatian bedtime stories — YouTube',
    web: 'https://www.youtube.com/results?search_query=price+za+laku+noc+hrvatska',
    ytId: 'RmkLbNe0JQo',
    icon: '🌙',
    color: '#7c3aed',
    cat: 'podcast',
    level: 'A1',
    tip: "Children's bedtime stories use simple, clear vocabulary read at perfect speed for beginners. Listening before sleep is proven to boost retention.",
  },
  {
    name: 'Croatian History Podcast',
    desc: 'Croatian history in podcast format',
    web: 'https://www.youtube.com/results?search_query=hrvatska+povijest+podcast',
    icon: '🏛️',
    color: '#b45309',
    cat: 'podcast',
    level: 'B2',
    tip: 'History podcasts use past tense extensively — the perfect practice ground for Croatian perfective/imperfective past tense in natural context.',
  },
  {
    name: 'Jezik i Komunikacija',
    desc: 'Croatian language tips & explanations',
    web: 'https://www.youtube.com/results?search_query=hrvatska+gramatika+objasnjenje+youtube',
    icon: '💬',
    color: '#0369a1',
    cat: 'podcast',
    level: 'B1',
    tip: 'Watch native speakers explain Croatian grammar in Croatian. At B1+ this is incredibly useful — you learn the language through the language itself.',
  },
  // ─── CULTURE ─────────────────────────────────────────────────────────────────
  {
    name: 'Pop Culture & Music',
    desc: 'Croatian pop culture, music scene & trends',
    web: '',
    icon: '🎵',
    color: '#7c3aed',
    cat: 'culture',
    level: 'B1',
    scr: 'popculture',
    tip: 'Understanding pop culture is the gateway to feeling at home in the language. Croatian memes, slang, and celebrity culture all reinforce casual registers.',
  },
  {
    name: 'Gloria.hr',
    desc: "Croatia's #1 lifestyle & celebrity magazine",
    web: 'https://www.gloria.hr/',
    icon: '💅',
    color: '#ec4899',
    cat: 'culture',
    level: 'B1',
    tip: 'Celebrity and lifestyle articles use colloquial vocabulary and informal sentence structures. Reading magazines makes your Croatian sound human, not textbook.',
  },
  {
    name: 'Slobodna Dalmacija',
    desc: 'Dalmatian newspaper since 1943',
    web: 'https://slobodnadalmacija.hr/',
    icon: '🌊',
    color: '#0284c7',
    cat: 'culture',
    level: 'B2',
    tip: 'Regional newspapers expose you to Dalmatian vocabulary and concerns. Understanding regional variation is key to C1+ comprehension.',
  },
  {
    name: 'Tportal.hr',
    desc: 'Technology & lifestyle portal',
    web: 'https://www.tportal.hr/',
    icon: '💻',
    color: '#0369a1',
    cat: 'culture',
    level: 'B1',
    tip: 'Tech articles introduce international vocabulary that you may already know in English — easier to learn Croatian terms when you already know the concept.',
  },
];
export const POPCULTURE = [
  // ytId: Baby Lasagna "Rim Tim Tagi Dim" — Eurovision 2024 semi-final official
  {
    name: 'Baby Lasagna',
    desc: 'Eurovision star! Gen-Z favorite',
    web: 'https://www.youtube.com/watch?v=drmJPqWmGbY',
    ytId: 'drmJPqWmGbY',
    icon: '⭐',
  },
  {
    name: 'Gibonni',
    desc: 'Legendary singer from Split',
    web: 'https://www.youtube.com/results?search_query=gibonni+najbolje+pjesme',
    ytId: '1Q5FiMMPBGw',
    icon: '🎵',
  },
  {
    name: 'Oliver Dragojević',
    desc: 'The voice of Dalmatia',
    web: 'https://www.youtube.com/results?search_query=oliver+dragojevic+cesarica',
    ytId: '9P_K0MBHuW8',
    icon: '🎵',
  },
  {
    name: 'Severina',
    desc: 'Croatian pop queen',
    web: 'https://www.youtube.com/results?search_query=severina+hitovi',
    icon: '🎵',
  },
  {
    name: 'Prljavo Kazalište',
    desc: 'Iconic Croatian rock',
    web: 'https://www.youtube.com/results?search_query=prljavo+kazaliste',
    icon: '🎸',
  },
  {
    name: 'Magazin',
    desc: 'Popular pop band',
    web: 'https://www.youtube.com/results?search_query=magazin+bend',
    icon: '🎵',
  },
  {
    name: 'Hladno Pivo',
    desc: 'Punk/rock from Zagreb',
    web: 'https://www.youtube.com/results?search_query=hladno+pivo+hitovi',
    icon: '🎸',
  },
  {
    name: 'Daleka Obala',
    desc: 'Split rock legends',
    web: 'https://www.youtube.com/results?search_query=daleka+obala',
    icon: '🎸',
  },
  // ytId: Let 3 "Mama ŠČ!" — Eurovision 2023 official
  {
    name: 'Let 3',
    desc: 'Art-rock, Eurovision 2023',
    web: 'https://www.youtube.com/watch?v=5KD5-Zi4VJ0',
    ytId: '5KD5-Zi4VJ0',
    icon: '🤘',
  },
  {
    name: 'Supertalent HR',
    desc: 'Croatian Got Talent',
    web: 'https://www.youtube.com/results?search_query=supertalent+hrvatska',
    icon: '📺',
  },
];
