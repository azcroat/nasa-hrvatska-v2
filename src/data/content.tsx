...
// are async/server paths or new dailyPickers utilities.
const { V, TOP100, BOJE, ALPHA, ZNAM } = _vocab;
const { HIST_FACTS, MAPPLACES, MEDIA, POPCULTURE } = _cultural;
const {
  PLACE,
  READ,
  UNJUMBLE,
...
const {
  ...
  AKUCLOTHES,
  CONVMATCH,
  // SP11e: TRANSPORT and GROCERY are still referenced elsewhere in this body
  // (legacy non-LEARN_PATH usage). FOODORDER's only previous use was the
  // V composition that's now server-side — removed from this destructure.
  TRANSPORT,
  GROCERY,
} = _scenarios;
...
// useContent() hook (components) or contentClient.getContent() (non-components).
// Removed from this re-export block: V, COUNTRIES, PROFESSIONS, WEATHER,
