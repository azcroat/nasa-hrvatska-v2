 ...
 *
 * PronounsScreen fires the contract when all pronoun case quiz questions are answered.
 * shMemo limits to 10 questions; we snapshot all gray-border option buttons and click them.
 */
...
describe('PronounsScreen contract (Pattern X)', () => {
  ...
  it('fires award, markQuest(grammar), setStats gc+1/vs:pronouns, writeDelta', async () => {
    ...
    render(
      <StatsProvider value={value}>
        <PronounsScreen goBack={vi.fn()} award={award} />
      </StatsProvider>,
    );
  ...
  it('is idempotent — skips setStats/writeDelta when vs already has pronouns', async () => {
    const { default: PronounsScreen } =
      await import('../components/practice/exercises/PronounsScreen');
    const { value, setStats, writeDelta, award } = makeCtx(['pronouns']);
