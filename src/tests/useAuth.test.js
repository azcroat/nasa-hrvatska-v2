...
vi.mock('firebase/auth', () => ({
  ...
  inMemoryPersistence:            {},
  signInWithEmailAndPassword:     vi.fn(() => Promise.resolve({ user: { email: 'x@y.com', uid: 'uid1', displayName: 'Test', emailVerified: true, providerData: [{ providerId: 'password' }] } })),
...
describe('useAuth — initial state', () => {
  ...
  it('setAuthError updates the error state', () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthError('Something went wrong'); });
    expect(result.current.authError).toBe('Something went wrong');
  });

  it('setAuthError can clear the error', () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthError('Error!'); });
    act(() => { result.current.setAuthError(''); });
    expect(result.current.authError).toBe('');
  });

  it('setEmailUnverified updates the flag', () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setEmailUnverified(true); });
...
describe('useAuth — doReg validation: email', () => {
  ...
  it('shows error when email is empty', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    await act(async () => { await result.current.doReg(); });
  ...
  it('shows error for invalid email format (missing @)', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthEmail('not-an-email'); });
    await act(async () => { await result.current.doReg(); });
    expect(result.current.authError).toBeTruthy();
    expect(result.current.authLoading).toBe(false);
  });
  ...
  it('shows error for invalid email format (missing domain)', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthEmail('user@'); });
    await act(async () => { await result.current.doReg(); });
    expect(result.current.authError).toBeTruthy();
  });
  ...
  it('clears authError before each doReg attempt', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthError('stale error'); });
    await act(async () => { await result.current.doReg(); }); // sets new error for empty email
...
describe('useAuth — doReg validation: password', () => {
  ...
  it('shows error when password is empty', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthEmail('user@example.com'); });
    await act(async () => { await result.current.doReg(); });
    expect(result.current.authError).toBeTruthy();
  });
...
describe('useAuth — doLog validation: email', () => {
  ...
  it('shows error when email is empty', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    await act(async () => { await result.current.doLog(); });
    expect(result.current.authError).toBeTruthy();
    expect(result.current.authLoading).toBe(false);
  });
  ...
  it('shows error for invalid email format', async () => {
    const { result } = renderHook(() => useAuth(mkCallbacks()));
    act(() => { result.current.setAuthEmail('notvalid'); });
    await act(async () => { await result.current.doLog(); });
    expect(result.current.authError).toBeTruthy();
  });
});
