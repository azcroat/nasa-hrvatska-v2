...
function buildMistakeSummary(srMistakes, majaPatterns, writingMistakes) {
  ...
  if (srMistakes && typeof srMistakes === 'object' && !Array.isArray(srMistakes)) {
    ...
    if (entries.length > 0) {
      const wordList = entries
        .map((e) => `${e.word}(${e.wrongCount}w/${e.rightCount}r)`)
        .join(', ');
  ...
  if (Array.isArray(majaPatterns) && majaPatterns.length > 0) {
    const safe = majaPatterns
      .slice(0, 20)
      .map((p) => sanitizeParam(String(p), 80))
      .filter(Boolean);
  ...
  if (Array.isArray(writingMistakes) && writingMistakes.length > 0) {
    const safe = writingMistakes
      .slice(0, 20)
      .map((m) => sanitizeParam(String(m), 200))
      .filter(Boolean);
...
export async function onRequestPost(context) {
  ...
  const safeSrMistakes =
    srMistakes && typeof srMistakes === 'object' && !Array.isArray(srMistakes) ? srMistakes : {};
    ...
    drills: Array.isArray(spot.drills)
      ? spot.drills.slice(0, 3).map((d) => ({
          ...
          options: Array.isArray(d.options)
            ? d.options.slice(0, 4).map((o) => String(o).trim())
            : [],
