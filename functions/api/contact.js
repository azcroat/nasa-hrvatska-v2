...
export async function onRequestPost(context) {
  ...
  if (!ADMIN_EMAIL) {
    ...
    return new Response(
      JSON.stringify({ ok: false, error: 'Contact service is temporarily unavailable.' }),
      {
        status: 500,
        headers: { ...CORS(origin), 'Content-Type': 'application/json' },
      },
  ...
  const replyBlock = replyEmail
    ...
    : `<div style="margin-top:20px;padding:14px 20px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;color:#94a3b8;font-size:13px">No reply email provided.</div>`;
