...
function fsVal(v) {
  ...
  if (v.mapValue)    { const m = {}; for (const k in (v.mapValue.fields || {})) m[k] = fsVal(v.mapValue.fields[k]); return m; }
  return null;
}

function intField(n)  { return { integerValue: String(Math.max(0, Math.floor(n))) }; }
function strField(s)  { return { stringValue: String(s) }; }
...
export async function onRequestPost(context) {
  ...
  if (!PROJECT_ID || !SA_JSON) {
    return new Response(
      JSON.stringify({ ok: false, error: 'Server misconfigured: missing FIREBASE_PROJECT_ID or FIREBASE_SERVICE_ACCOUNT_JSON' }),
  ...
  let token;
  try { token = await getFirestoreToken(SA_JSON); }
  catch (e) {
    return new Response(
      JSON.stringify({ ok: false, error: `Token error: ${e.message}` }),
      { status: 500, headers: { 'Content-Type': 'application/json' } },
    );
  }
  ...
  try {
    ...
    if (!listRes.ok) {
      ...
      return new Response(
        JSON.stringify({ ok: false, error: `Firestore list failed ${listRes.status}: ${errText.slice(0, 200)}` }),
    ...
    return new Response(
      JSON.stringify({ ok: false, error: `List error: ${e.message}` }),
  ...
  for (const doc of docs) {
    ...
    try {
      ...
      if (!progressStr || typeof progressStr !== 'string') { skipped++; continue; }

      let progress;
      try { progress = JSON.parse(progressStr); }
      catch { errors++; errorLog.push(`${doc.name}: corrupted JSON`); continue; }
      ...
      const blobStreak = progress.streak && typeof progress.streak.count === 'number'
      ...
      const blobNeedsUpdate = reconcXP !== blobXP || reconcLC !== blobLC || reconcStreak !== blobStreak;
      const topNeedsUpdate  = reconcXP !== topXP  || reconcLC !== topLC  || reconcStreak !== topStreak;

      if (!blobNeedsUpdate && !topNeedsUpdate) { skipped++; continue; }
