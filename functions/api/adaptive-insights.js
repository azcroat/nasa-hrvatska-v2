...
function staticFallback(origin) {
  return ok({
    weakAreas: [
      { area: "Verbal aspect", severity: "high", explanation: "Imperfective vs. perfective verb pairs are one of the most common challenges for English speakers learning Croatian." },
      { area: "Case endings", severity: "medium", explanation: "Noun and adjective declension takes consistent practice to internalize." },
      { area: "Vocabulary retention", severity: "low", explanation: "Regular SRS review sessions will help lock in new words." },
    ],
    todaysFocus: "Focus on reviewing your flashcard deck and practice verb aspects in context.",
    drillSuggestions: [
      { type: "Aspect drill", description: "Choose the correct verb aspect for each sentence", screen: "aspect" },
      { type: "Flashcard review", description: "Review your weakest SRS cards", screen: "flashcards" },
      { type: "Case practice", description: "Practice noun declension with common words", screen: "declension" },
    ],
...
export async function onRequestPost({ request, env }) {
  ...
  const safeLessonsCompleted = Number.isFinite(lessonsCompleted) ? Math.max(0, Math.floor(lessonsCompleted)) : 0;

  const safeRecentTopics = Array.isArray(recentTopics)
    ? recentTopics.slice(0, 10).map(t => sanitizeParam(String(t), 60)).filter(Boolean)
    : [];
  ...
  const weakAreas = Array.isArray(parsed.weakAreas)
    ? parsed.weakAreas.slice(0, 3).map(a => ({
  ...
  const drillSuggestions = Array.isArray(parsed.drillSuggestions)
    ? parsed.drillSuggestions.slice(0, 3).map(d => ({
