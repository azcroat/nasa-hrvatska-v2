...
function detectConfusion(text) {
  const confusionPhrases =
    /\b(ne\s+razumijem|not\s+understand|don't\s+understand|confused|what\s+does|što\s+znači|what\s+mean|pomozi|help\s+me|ne\s+znam|i\s+don't\s+know|lost|ne\s+mogu)\b/i;
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const effectiveBreakdown =
      userConfused || userLang === 'en' ? Math.min(breakdownCount + 1, 3) : breakdownCount;

    const sanitizePersonaField = (v, max) =>
      v
        ? String(v)
            .replace(/[\r\n`\\]/g, ' ')
