...
Return ONLY valid JSON, no markdown.`;
  ...
  try {
    ...
    console.error(
      'flash-context.js: JSON parse failed on Anthropic response:',
      rawBody.slice(0, 200),
    );
  ...
  const note =
    typeof parsed.note === 'string' && parsed.note.trim() ? parsed.note.trim().slice(0, 80) : null;
  const mnemonic =
    typeof parsed.mnemonic === 'string' && parsed.mnemonic.trim()
      ? parsed.mnemonic.trim().slice(0, 200)
      : null;
