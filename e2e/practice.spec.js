...
test.describe('Practice tab structure', () => {
  ...
  test('Challenge panel shows Quick Games and AI Challenges sections', async ({ page }) => {
    await page.locator('button').filter({ hasText: /^Quick Game$/ }).click();
...
test.describe('ClozeEngine — Sentence Cloze', () => {
  ...
  test('four answer buttons are shown in a 2x2 grid', async ({ page }) => {
    ...
    const optionGrid = page.locator('div').filter({
  ...
  test('selecting a correct answer shows "✓ Correct!" feedback', async ({ page }) => {
    ...
    const feedback = page.locator('div').filter({ hasText: /✓ Correct!|✗ The answer was/ }).first();
  ...
  test('wrong answer shows "Why is … correct?" AI explain button', async ({ page }) => {
    ...
    const isWrong = await page.getByText(/✗ The answer was/).isVisible({ timeout: 2_000 }).catch(() => false);
    if (!isWrong) {
      // Was correct — advance and try another question's options
      await page.locator('button.b.bp').filter({ hasText: /Next →/ }).click();
      await page.waitForTimeout(200);
  ...
  test('Next → button advances to question 2 after answering', async ({ page }) => {
    ...
    await expect(page.locator('button.b.bp').filter({ hasText: /Next →/ })).toBeVisible({ timeout: 3_000 });
    await page.locator('button.b.bp').filter({ hasText: /Next →/ }).click();
...
test.describe('DictationScreen', () => {
  ...
  test('Dictation card is visible inside Advanced category', async ({ page }) => {
    ...
    await expect(page.locator('button.exercise-card').filter({ hasText: 'Dictation' })).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('text input with placeholder "Type what you heard..." is visible', async ({ page }) => {
    ...
    await page.locator('button.exercise-card').filter({ hasText: 'Dictation' }).click();
    await expect(page.getByText('🎧 Dictation')).toBeVisible({ timeout: 8_000 });
    await expect(page.locator('input[placeholder="Type what you heard..."]')).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('all 10 diacritic buttons are rendered', async ({ page }) => {
    ...
    for (const ch of ['Č', 'Ć', 'Š', 'Ž', 'Đ', 'č', 'ć', 'š', 'ž', 'đ']) {
      await expect(page.locator('button').filter({ hasText: ch }).first()).toBeVisible({ timeout: 3_000 });
    }
  });
  ...
  test('Check ✓ button is visible before submitting', async ({ page }) => {
    ...
    await page.locator('button.exercise-card').filter({ hasText: 'Dictation' }).click();
    await expect(page.getByText('🎧 Dictation')).toBeVisible({ timeout: 8_000 });
    await expect(page.locator('button.b.bp').filter({ hasText: /Check ✓/ })).toBeVisible({ timeout: 5_000 });
  });

  test('typing a wrong answer and checking reveals "Correct answer:" and the Explain button', async ({ page }) => {
    await openCategory(page, 'Advanced');
    await page.locator('button.exercise-card').filter({ hasText: 'Dictation' }).click();
    await expect(page.getByText('🎧 Dictation')).toBeVisible({ timeout: 8_000 });
    ...
    await page.locator('button.b.bp').filter({ hasText: /Check ✓/ }).click();
    ...
    await expect(page.locator('button').filter({ hasText: '🧠 Explain this sentence' })).toBeVisible({ timeout: 5_000 });
  });
...
test.describe('AIListeningScreen', () => {
  ...
  test('all 10 topic buttons are visible', async ({ page }) => {
    ...
    const topicTexts = ['U kafiću', 'Na tržnici', 'Obitelj', 'Putovanje', 'Vrijeme', 'Sport', 'Posao', 'Vikend', 'Restoran', 'Grad'];
    for (const topic of topicTexts) {
      await expect(page.locator('button').filter({ hasText: topic })).toBeVisible({ timeout: 5_000 });
    }
  });
  ...
  test('clicking Generate → transitions to the listening phase showing content title', async ({ page }) => {
    ...
    await page.locator('button').filter({ hasText: 'U kafiću' }).click();
    await page.locator('button.b.bp').filter({ hasText: /Generate →/ }).click();
  ...
  test('listening phase shows Play/Pause audio control button', async ({ page }) => {
    ...
    await expect(page.locator('button[aria-label="Play audio"], button[aria-label="Pause audio"]'))
  ...
  test('listening phase has Show Transcript / Hide Transcript toggle', async ({ page }) => {
    ...
    await expect(page.getByText(/Show Transcript|Hide Transcript/)).toBeVisible({ timeout: 10_000 });
  });
...
test.describe('VideoLessonScreen', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/scene-video**', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  test('setup screen shows "Difficulty Level" selector with A1–C2 buttons', async ({ page }) => {
    ...
    for (const lvl of ['A1', 'A2', 'B1', 'B2', 'C1', 'C2']) {
      await expect(page.locator('button').filter({ hasText: lvl }).last()).toBeVisible({ timeout: 3_000 });
    }
  });
  ...
  test('selecting a topic enables the Start button', async ({ page }) => {
    ...
    await expect(page.getByText('🎬 Video Lesson')).toBeVisible({ timeout: 8_000 });
    await page.locator('button').filter({ hasText: 'U kafiću' }).first().click();
    await expect(page.locator('button').filter({ hasText: /▶ Start Lesson/ })).toBeEnabled({ timeout: 5_000 });
  });
  ...
  test('generating shows the playing phase with "▶ Play Dialogue" button', async ({ page }) => {
    ...
    await expect(page.getByText('🎬 Video Lesson')).toBeVisible({ timeout: 8_000 });
    await page.locator('button').filter({ hasText: 'U kafiću' }).first().click();
    await page.locator('button').filter({ hasText: /▶ Start Lesson/ }).click();
...
test.describe('SpeedChallenge — Practice tab', () => {
  ...
  test('"Play →" button is visible when not played today', async ({ page }) => {
    await expect(page.locator('button').filter({ hasText: /^Play →$/ })).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('clicking "Play →" shows the first question (a Croatian word prompt)', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ hasText: /^Play →$/ })).toBeHidden({ timeout: 5_000 });
